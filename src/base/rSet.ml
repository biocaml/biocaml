...
let union s t = of_interval_list (s @ t) (* better implementation possible *)
            ...
            match Range.intersect u v with
              ...
              | Some w ->
                  match Pervasives.compare u.Range.hi v.Range.hi with
                    ...
                    |  _ -> invalid_arg "impossible to get here"
  in
    of_interval_list (loop [] s t) (* calling of_interval_list assures canoninicity, but maybe could get that simply by List.rev *)
...
module Test = struct
  let test vl1 vl2 =
    ...
    let set1 = Test.timesf "making first efficient set" of_interval_list vl1 in
    let set2 = Test.timesf "making second efficient set" of_interval_list vl2 in
