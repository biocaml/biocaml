...
let load ~on_output ?(max_read_bytes = Int.max_value) filename =
  ...
  let parsing_transform =
    match tags with
    | `bed ->
      Biocaml_transform.on_output (Biocaml_bed.Transform.string_to_item ()) ~f:(function
        | Ok o -> Ok o
        | Error e -> Error (`bed e))
    | `gzip `bed ->
    ...
    | _ -> failwith "cannot handle file-format"
  in
...
let load_itree () =
  ...
  let add n low high content =
    ...
    | Some tree_ref ->
      tree_ref
        := Biocaml_interval_tree.add !tree_ref ~low ~high ~data:(n, low, high, content)
    | None ->
...
let say fmt = ksprintf (fun s -> wrap_deferred_lwt (fun () -> Lwt_io.print s)) fmt
...
let intersects map name low high =
  ...
  | None -> say "Record for %S not found\n" name
;;
...
let rset_folding ~fold_operation ~fold_init max_read_bytes input_files =
  ...
  List.fold input_files ~init:(return []) ~f:(fun prev file ->
    ...
    >>= fun () ->
    all_names := Set.union !all_names (String.Set.of_list (Map.keys !map_ref));
    return (!map_ref :: current_list))
  >>= fun files_maps ->
  Set.fold !all_names ~init:(return ()) ~f:(fun munit name ->
    munit
    >>= fun () ->
    List.filter_map files_maps (fun fm -> Map.find fm name |> Option.map ~f:( ! ))
...
let command =
  Command_line.(
    group
      ~summary:"Operations on BED files (potentially gzipped)"
      [ ( "intersects"
        , basic
            ...
            (fun max_read_bytes input_file name start stop ->
              ...
              >>= (fun () -> intersects !map_ref name start stop)
            ...
            (fun max_read_bytes input_file ->
              ...
              >>= (fun () ->
                    Map.fold !map_ref ~init:(return ()) ~f:(fun ~key ~data m ->
                      ...
                      >>= fun () ->
                      List.fold
                        (Biocaml_rSet.to_range_list !data)
                        ~init:(return ())
                        ~f:(fun m (low, high) ->
                        m >>= fun () -> say "%s\t%d\t%d\n" key low high)))
              >>< common_error_to_string) )
