...
type barcode_specification =
  { barcode : string
  ; position : int
  ; on_read : int (** [mismatch]: [None] means “leave decision to application defaults” *)
  ; mismatch : int option
  }
...
let barcode_specification_of_sexp : Sexp.t -> barcode_specification =
  ...
  | Atom all_in_one as sexp -> (
    match String.split ~on:':' all_in_one with
...
let sexp_of_barcode_specification { mismatch; barcode; position; on_read } =
  ...
  let more =
    match mismatch with
    | None -> []
    | Some mm -> [ Atom "with"; Atom "mismatch"; Atom (Int.to_string mm) ]
  in
  List
    (Atom barcode
    :: Atom "on"
    :: Atom "read"
    :: Atom (Int.to_string on_read)
    :: Atom "at"
    :: Atom "position"
    :: Atom (Int.to_string position)
    :: more)
;;
...
type library =
  { name_prefix : string
  ; filter : [ `barcoding of barcode_specification Blang.t | `undetermined ]
  }
...
let library_of_sexp =
  ...
  | List [ Atom "library"; Atom name_prefix; List [ Atom "barcoding"; sexp ] ] ->
    { name_prefix
    ; filter = `barcoding (Blang.t_of_sexp barcode_specification_of_sexp sexp)
    }
  | List [ Atom "library"; Atom name_prefix; Atom "undetermined" ] ->
    { name_prefix; filter = `undetermined }
  | sexp -> of_sexp_error (sprintf "wrong library") sexp
;;
...
let sexp_of_library { name_prefix; filter } =
  ...
  let rest =
    match filter with
    | `barcoding barcoding ->
      List [ Atom "barcoding"; Blang.sexp_of_t sexp_of_barcode_specification barcoding ]
    | `undetermined -> Atom "undetermined"
  in
...
let sexp_of_demux_specification l =
  ...
  | `inclusive -> List (Atom "demux" :: List.map l.demux_rules sexp_of_library)
  | `exclusive -> List (Atom "exclusive-demux" :: List.map l.demux_rules sexp_of_library)
;;
...
(** [check_barcode] is the function that tells if a
   ...
   was needed. *)
let check_barcode ~mismatch ~position ~barcode (sequence : string) : bool * int =
  let allowed_mismatch = ref mismatch in
  let index = ref 0 in
  while !allowed_mismatch >= 0 && !index <= String.length barcode - 1 do
    if sequence.[position - 1 + !index] <> barcode.[!index] then decr allowed_mismatch;
    incr index
  done;
...
let string_of_error e =
  Sexp.to_string_hum
    ([%sexp_of: [ Biocaml_fastq.Err.t | `unzip of Biocaml_zip.Error.unzip ]] e)
;;
...
let library_statistics () = { read_count = 0; no_mismatch_read_count = 0 }
...
let perform ~mismatch ?do_statistics ~read_files ~demux_specification =
  ...
  *)
      ...
      | (Ok (`gzip `fastq) | _) when String.is_suffix filename ".gz" ->
        Biocaml_transform.compose_results
          ...
          (Biocaml_zip.Transform.unzip
             ~zlib_buffer_size:(3 * input_buffer_size)
             ~format:`gzip
             ())
          (Biocaml_fastq.Transform.string_to_item ~filename ())
      | _ -> Biocaml_fastq.Transform.string_to_item ~filename ()
    in
  ...
  (match errors with
   | [] -> return ()
   | some -> close_all_inputs () >>= fun () -> error (`openning_files some))
  >>= fun () ->
  ...
  for_concurrent demux_specification.demux_rules (fun { name_prefix; filter } ->
    for_concurrent_with_index read_files (fun i _ ->
      ...
      in
      wrap_deferred_lwt (fun () ->
        Lwt_io.(open_file ~mode:output ~buffer_size:output_buffer_size actual_filename))
      >>= fun o -> return (transform, o))
    >>= function
    | outs, [] ->
      return
        ( name_prefix
    ...
    | outs, some_errors ->
      close_all_inputs ()
      >>= fun () ->
      while_sequential outs (fun (_, o) -> wrap_deferred_lwt (fun () -> Lwt_io.close o))
      >>= fun _ -> error (`openning_files some_errors))
    ...
    then return ()
          ...
          let matches =
            ...
            | `barcoding barcoding ->
              Blang.eval barcoding (fun { on_read; barcode; position; mismatch } ->
                let mismatch = Option.value ~default:default_mismatch mismatch in
                try
                  let item = List.nth_exn items (on_read - 1) in
                  let matches, mm =
                    check_barcode ~position ~barcode ~mismatch item.Biocaml_fastq.sequence
                  in
      ...
      for_concurrent matches_also_policy (fun (name, max_mismatch, outs, stats_opt) ->
        Option.iter stats_opt (fun s ->
          ...
          if max_mismatch = 0
          then s.no_mismatch_read_count <- s.no_mismatch_read_count + 1);
        List.fold2_exn
          outs
          items
          ~init:(return ())
          ~f:(fun m (transform, out_channel) item ->
          m >>= fun () -> push_to_the_max ~transform ~out_channel item))
      >>= fun (_, errors) -> check_errors errors >>= fun () -> loop ()
  in
  loop ()
  >>= fun () ->
  (* Optionally open the statistics file. *)
  (match do_statistics with
   | Some s ->
     let open Lwt_io in
     wrap_deferred_lwt (fun () ->
       open_file ~mode:output ~buffer_size:output_buffer_size s)
     >>= fun o ->
     wrap_deferred_lwt (fun () ->
  ...
  for_concurrent output_specs (fun (name, os, spec, stats) ->
    for_concurrent os ~f:(fun (transform, out_channel) ->
      ...
      >>= fun () -> wrap_deferred_lwt (fun () -> Lwt_io.close out_channel))
    >>= fun (_, errors) ->
    (* TODO: check errors *)
    match stats, stats_channel with
    | Some { read_count; no_mismatch_read_count }, Some o ->
      wrap_deferred_lwt (fun () ->
        Lwt_io.fprintf o "(%S %d %d)\n" name read_count no_mismatch_read_count)
    | _, _ -> return ())
  >>= fun _ ->
  ...
  >>= fun (_, errors) ->
  (* TODO: check errors *)
  Option.value_map stats_channel ~default:(return ()) ~f:(fun c ->
...
let parse_configuration s =
  ...
  let mismatch =
    List.find_map entries (function
      | List [ Atom "default-mismatch"; Atom vs ] -> Some (Int.of_string vs)
      | _ -> None)
  in
  let gzip =
    List.find_map entries (function
      | List [ Atom "gzip-output"; Atom vs ] -> Some (Int.of_string vs)
      | _ -> None)
  in
  let demux =
    List.find_map entries (function
      | List (Atom "demux" :: _) as s -> Some (demux_specification_of_sexp s)
      | List (Atom "exclusive-demux" :: _) as s -> Some (demux_specification_of_sexp s)
      | _ -> None)
  in
  let stats =
    List.find_map entries (function
      | List [ Atom "statistics"; Atom vs ] -> Some vs
      | _ -> None)
  in
  let inputs =
    List.find_map entries (function
      | List (Atom "input" :: files) ->
        Some
          (List.map files (function
            | Atom a -> a
            | s -> failwithf "wrong input files specification: %s" (to_string_hum s) ()))
      | _ -> None)
  in
...
let more_help original_help : string =
  ...
  let ex ?(demux_policy = `inclusive) v =
    ...
    Sexp.to_string_hum (sexp_of_demux_specification incl)
  in
  ...
  ul
    buf
    [ "`specification` is either `-specification <file>`, `-demux <string>`, or \
       `-exclusive-demux <string>`. A specification file containing `(demux <string>)` \
       is equivalent to `-demux <string>` (idem for `eclusive-demux`)."
    ; "`R1`, `R2`, … are (potentially gzipped) fastq files containing the reads to \
       demultiplex"
  ...
  par
    buf
    "The file is a sequence of S-Expressions (c.f. [W:S-Expression])\n\
  ...
  par
    buf
    "Here is a detailed example of specification file (meant to be\n\
  ...
  code
    ...
    (ex
       [ { name_prefix = "LibONE"
         ; filter =
             `barcoding
               (barcode_specification ~position:1 "ACTGTT" ~mismatch:1 ~on_read:2)
         }
       ; { name_prefix = "LibTWO"
         ; filter =
             `barcoding
               (barcode_specification ~position:1 "CTTATG" ~mismatch:1 ~on_read:2)
         }
       ]);
    ...
    (ex
       [ { name_prefix = "LibAND"
         ; filter =
             `barcoding
               (and_
                  [ barcode_specification ~position:5 "ACTGTT" ~mismatch:1 ~on_read:1
  ...
  code
    ...
    (ex
       ...
       ; { name_prefix = "LibUndetermined"; filter = `undetermined }
       ]);
  par
    buf
    "→ assuming that default-mismatch is 1, in this case, a read of\n\
  ...
  let example =
    "(demux\n\
    \  (library \"Lib with ånnœ¥ing name\" (barcoding ACCCT:1:2)) ;; one barcode on R1, \
     at pos 2\n\
    \  (library GetALL (barcoding true)) ;; get everything\n\
    \  (library Merge (barcoding (and AGTT:2:42:1 ACCC:2:42:1 \n\
    \                   (not AGTGGTC:1:1:2))) ;; a ∧ b ∧ ¬c  matching\n\
     ))"
  in
...
let command =
  Command_line.(
    basic
      ~summary:"Fastq deumltiplexer"
      ~readme:(fun () -> "See option [-manual] for more detailed explanations.")
      Spec.(
        file_to_file_flags ()
        ...
        +> help
        ++ uses_lwt ())
      (fun mismatch_cl indemux_cl exdemux_cl spec stats_cl ~manual read_files_cl help ->
        (if manual
        then Say.raw "%s%!" (more_help (Lazy.force help))
        else
          (match spec with
           | Some s ->
             wrap_deferred_lwt (fun () ->
          ...
          (match read_files_cl, inputs with
           | [], Some l -> return l
           | l, None -> return l
           | l, Some ll ->
             failf "conflict: input files defined in command line and configuration file")
          >>= fun read_files ->
          let mismatch =
          ...
          let () =
            Option.iter gzip_conf (fun level ->
              Global_configuration.gzip_set_if_not_set ~level ())
          in
          let do_statistics = if stats_cl <> None then stats_cl else stats in
          let parse s =
            try return (Sexp.of_string s |> demux_specification_of_sexp) with
            | e -> failf "Parsing error: %s\n%!" (Exn.to_string e)
          in
