...
let of_buffer b = try Some (of_buffer_exn b) with Bad _ -> None
...
let of_string b = try Some (of_string_exn b) with Bad _ -> None
...
let slice first last t =
  if first < 0 || last >= String.length t || first > last
  then failwith "requesting invalid slice from sequence"
  else let last = last+1 in String.slice ~first ~last t
