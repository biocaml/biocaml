...
val mkdir_safe : ?perm:Unix.file_perm -> string -> unit
  (** [mkdir_safe dir] will make directory [dir] with permissions 750 by default. If the directory previously exists, the permissions will be set to those requested. Will raise [Failure] if the path previously exists but is not a directory. *)
