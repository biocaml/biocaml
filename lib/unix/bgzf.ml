  ...
  | Some id1 -> (
      try
        ...
        if id1 <> 0x1F || id2 <> 0x8B then
          raise (Error "bad magic number, not a bgzf file");
        ...
        if si1 <> 66 || si2 <> 67 || slen <> 2 then
          raise (Error "bad extra subfield");
        ...
        bsize - xlen - 19
      with End_of_file ->
        raise (Error "premature end of file, not a bgzf file"))

let read_block iz =
  let rec loop posz lenz pos len crc size =
    let finished, used_in, used_out =
      try
        Zlib.inflate iz.in_stream iz.in_bufz posz lenz iz.in_buf pos len
          Zlib.Z_SYNC_FLUSH
      with Zlib.Error (_, _) -> raise (Error "error during decompression")
    in
...
let input iz buf pos len =
  ...
  if pos < 0 || len < 0 || pos + len > n then
    raise (Invalid_argument "Bgzf.input");
  if iz.in_eof then 0
  else
...
let input_string iz n =
  if n < 0 then
    raise (Invalid_argument "Bgzf.input_string iz n: n should be non negative");
...
let virtual_offset iz =
  if iz.in_pos = iz.in_avail then Int64.(shift_left (In_channel.pos iz.ic) 16)
  else Int64.(shift_left iz.in_block_offset 16 + of_int_exn iz.in_pos)

let with_file_in fn ~f =
  let iz = open_in fn in
  let r = try `Ok (f iz) with e -> `Error e in
  close_in iz;
  match r with `Ok y -> y | `Error exn -> raise exn
...
let of_out_channel ?(level = 6) oc =
  if level < 1 || level > 9 then
    raise (invalid_arg "Bgzf: bad compression level");
  {
    out_chan = oc;
...
let push_block oz =
  ...
  let _, used_in, used_out =
    try
      Zlib.deflate stream oz.out_ubuffer 0 oz.out_pos oz.out_cbuffer 0
        (Bytes.length oz.out_cbuffer)
        Zlib.Z_FINISH
    with Zlib.Error (_, _) ->
      raise (Unparser_error "error during compression")
  in
...
let rec output ~length ~blit oz buf ~pos ~len =
  ...
  if remaining > 0 then
    output ~length ~blit oz buf ~pos:(pos + ncopy) ~len:remaining

let output_from_string =
  output ~length:String.length ~blit:Caml.Bytes.blit_string
...
let with_file_out ?level fn ~f =
  ...
  let r = try `Ok (f oz) with e -> `Error e in
  close_out oz;
  match r with `Ok y -> y | `Error exn -> raise exn
