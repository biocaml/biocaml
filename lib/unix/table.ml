module Row = struct
  ...
  module Tags = struct
    ...
    let separators tags =
      List.filter_map tags ~f:(function `separator c -> Some c | _ -> None)
  ...
  let of_line ?(separators = [ ' '; '\t' ]) ?(strict_row_length = false)
    ...
    let module With_exns = struct
      ...
      let int s = try Int.of_string s with _ -> raise (Int_of_string s)
      let float s = try Float.of_string s with _ -> raise (Float_of_string s)
                ...
                let row =
                  Array.mapi tokens ~f:(fun i tok ->
                      let typ =
                        ...
                        else try format.(i) with _ -> `type_string
                      in
                      match typ with
