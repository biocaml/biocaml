...
module Buffer = struct
  ...
  let make ?filename () =
    { unfinished_line = None; lines = Queue.create (); parsed_lines = 0; filename }
  ;;
...
module Transform = struct
  ...
  let make ?name ?filename ~next ~on_error () =
    ...
    Tfxm.make ?name () ~feed:(Buffer.feed_string lo_parser) ~next:(fun stopped ->
