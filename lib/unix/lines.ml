...
let with_file fn ~f = In_channel.with_file fn ~f:(fun ic -> f (read ic))
...
module Buffer = struct
  ...
  let feed_line p s = Queue.enqueue p.lines s
  ...
  let next_line p =
    ...
    if Poly.(l <> None) then p.parsed_lines <- p.parsed_lines + 1;
    l

  exception No_next_line

  let next_line_exn p =
    match next_line p with Some s -> s | None -> raise No_next_line
...
module Transform = struct
  let string_to_item () =
    ...
    Tfxm.make ~name:"string_to_lines" ~feed:(Buffer.feed_string buf)
      ~next:(function
        | true -> (
  ...
  let item_to_string ?(buffer : [ `clear of int | `reset of int ] = `reset 1024)
    ...
    let buffer, clear_buffer =
      ...
      | `reset s -> (Buffer.create s, Buffer.reset)
    in
  ...
  let make ?name ?filename ~next ~on_error () =
    ...
    Tfxm.make ?name () ~feed:(Buffer.feed_string lo_parser)
      ~next:(fun stopped ->
        ...
        | `not_ready ->
            if stopped then
              if Buffer.is_empty lo_parser then `end_of_stream
              else
