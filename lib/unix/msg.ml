let msg ?(pre = "MSG") ?pos msg =
  match pos with
  | None -> pre ^ ": " ^ msg
  | Some p -> pre ^ "[" ^ Biocaml.Pos.to_string p ^ "] " ^ msg
;;
...
let max_array_length_error =
  "Out of memory, possibly because trying to construct array of size greater than "
  ^ string_of_int Array.max_length
;;
