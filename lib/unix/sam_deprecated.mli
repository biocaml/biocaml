...
(** Here we expose functions used both in {!Sam_deprecated.Transform} and
      {!Bam.Transform} for parsing.
...
module Transform: sig
  ...
  val string_to_raw: ?filename:string -> unit ->
    (string, (raw_item, [> Error.string_to_raw]) Result.t) Tfxm.t
  ...
  val item_to_raw: unit ->
    (item, (raw_item, [> Error.item_to_raw]) Result.t) Tfxm.t
