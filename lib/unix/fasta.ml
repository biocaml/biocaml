...
let sequence_to_int_list s =
  ...
  |> Result.List.map ~f:(fun x ->
       ...
       | Failure _ -> error "invalid int" x sexp_of_string)
;;
...
let parse_item0
  ...
  let s = (line : Biocaml.Line.t :> string) in
  ...
  (match max_line_length with
   ...
   | Some x ->
     if x <= n
     then Ok ()
     else error "max_line_length exceeded" (x, n) [%sexp_of: int * int])
  >>= fun () ->
  if allow_empty_lines && String.for_all s ~f:Char.is_whitespace
  then Ok `Empty_line
  else if (not allow_empty_lines) && n = 0
  then
    Or_error.error_string "allow_empty_lines is false but got empty line"
    (* n > 0 if we got here *)
  else if Char.equal s.[0] '>'
  then Ok (`Description (String.slice s 1 n))
  else (
    match allow_sharp_comments, allow_semicolon_comments, s.[0] with
    | true, true, (';' | '#') | true, false, '#' | false, true, ';' ->
      Ok (`Comment (String.slice s 1 n))
    | false, false, (';' | '#') -> Or_error.error_string "comments lines are not allowed"
    | _ -> (
      match alphabet with
      | None -> Ok (`Partial_sequence s)
      | Some alphabet ->
        if String.for_all s ~f:(String.mem alphabet)
        then Ok (`Partial_sequence s)
        else
          (* TODO: report which character is outside alphabet *)
          error
            "sequence contains string outside allowed alphabet"
            (s, alphabet)
...
let read0
  ?(start = Biocaml.Pos.(incr_line unknown))
  ?(allow_sharp_comments = true)
  ?(allow_semicolon_comments = false)
  ?(allow_empty_lines = false)
  ?max_line_length
  ...
  CFStream.Stream.map (Lines.read r) ~f:(fun line ->
    ...
    parse_item0
      ...
      ?max_line_length
      ?alphabet
    |> fun x -> Or_error.tag_arg x "position" current_pos Biocaml.Pos.sexp_of_t)
;;
...
let read_header ?(allow_empty_lines = false) (item0s : item0 Or_error.t Stream.t)
  ...
  let rec loop accum : header Or_error.t =
    ...
    | Some (Ok `Empty_line) ->
      ...
      then loop accum
      else Or_error.error_string "allow_empty_lines is false but got empty line in header"
    | Some (Ok (`Description _)) | Some (Ok (`Partial_sequence _)) -> Ok accum
    | Some (Error _ as e) -> e
    | None -> Ok accum
  in
    ...
    let rec f description partial_seqs : item Or_error.t option =
      ...
      | Some (Ok (`Description x)) -> (
        ...
        | None, _ :: _ ->
          ...
          assert false
        | Some _, [] -> error_string "previous description line not followed by sequence"
      ...
      | None -> (
        ...
        | None, _ :: _ ->
          ...
          assert false
        | Some _, [] ->
          error_string "description line not followed by sequence, reached end-of-file"
        | Some description, partial_seqs ->
          Some
            (Ok
               { description
...
let with_file ?fmt file ~f =
  let start = Biocaml.Pos.make ~source:file ~line:1 () in
