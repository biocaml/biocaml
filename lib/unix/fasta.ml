...
type item = { description : string; sequence : string }
...
(* Low-level Parsing                                                          *)
(******************************************************************************)
let parse_item0 ?(allow_sharp_comments = true)
    ?(allow_semicolon_comments = false) ?(allow_empty_lines = false)
    ?max_line_length ?alphabet line =
  ...
  if allow_empty_lines && String.for_all s ~f:Char.is_whitespace then
    ...
    (* n > 0 if we got here *)
  else if Char.equal s.[0] '>' then Ok (`Description (String.slice s 1 n))
  else
...
let read0 ?(start = Pos.(incr_line unknown)) ?(allow_sharp_comments = true)
    ?(allow_semicolon_comments = false) ?(allow_empty_lines = false)
    ?max_line_length ?alphabet r =
  let pos = ref start in
  Stream.map (Lines.read r) ~f:(fun line ->
      let current_pos = !pos in
      pos := Pos.incr_line !pos;
      parse_item0 line ~allow_sharp_comments ~allow_semicolon_comments
        ~allow_empty_lines ?max_line_length ?alphabet
      |> fun x -> Or_error.tag_arg x "position" current_pos Pos.sexp_of_t)
...
let read_header ?(allow_empty_lines = false)
    (item0s : item0 Or_error.t Stream.t) : header Or_error.t =
  let rec loop accum : header Or_error.t =
    ...
    | Some (Ok (`Comment x)) ->
        ...
        loop (x :: accum)
    | Some (Ok `Empty_line) ->
        if allow_empty_lines then loop accum
        else
...
let read ?start ?(fmt = default_fmt) r =
  ...
  let item0s =
    read0 r ?start ~allow_sharp_comments ~allow_semicolon_comments
      ~allow_empty_lines ?max_line_length ?alphabet
  in
  ...
  | Ok header ->
      let rec f description partial_seqs : item Or_error.t option =
        ...
        | Some (Ok (`Description x)) -> (
            ...
            | Some _, [] ->
                error_string
                  "previous description line not followed by sequence"
                  ...
                  (Ok
                     {
                       description;
                       sequence =
                         partial_seqs |> List.rev |> String.concat ~sep:"";
                     }))
        ...
        | None -> (
            ...
            | Some _, [] ->
                error_string
                  "description line not followed by sequence, reached \
                   end-of-file"
            | Some description, partial_seqs ->
                Some
                  (Ok
