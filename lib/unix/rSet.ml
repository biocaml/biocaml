...
let to_canonical (vl : Range.t list) : Range.t list =
  ...
  let rec canonize ans vl =
    ...
    | v :: [] -> v :: ans
    | u :: (v :: vl as tail) ->
        if Poly.equal u v then canonize ans tail
        else if Range.superset u v then canonize ans (u :: vl)
...
let inter s t =
  let rec loop ans s t =
    ...
    | [], _ -> ans
    | (u :: s as ul), (v :: t as vl) -> (
        if u.Range.lo > v.Range.hi then loop ans ul t
        else if u.Range.hi < v.Range.lo then loop ans s vl
        else
...
let diff s t =
  let rec loop ans s t =
    ...
    | (u :: s as ul), (v :: t as vl) -> (
        if u.Range.lo > v.Range.hi then loop ans ul t
        else if u.Range.hi < v.Range.lo then loop (u :: ans) s vl
        else
          match Range.intersect u v with
          | None -> invalid_arg "impossible to get here"
          | Some w ->
