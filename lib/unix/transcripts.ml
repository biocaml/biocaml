...
let of_composite_channel ?(chr_map = identity) ?(increment_lo_hi = (0, 0)) ic =
  let f acc l =
    ...
    in
    let g (nm, chr, st, fn) prev =
      match prev with
      | None -> { exons = [ (st, fn) ]; lo = st; hi = fn; chr; info = nm }
      | Some trx ->
          {
...
let of_composite_file ?(chr_map = identity) ?(increment_lo_hi = (0, 0)) file =
  try_finally
    (of_composite_channel ~chr_map ~increment_lo_hi)
...
let of_bed_channel ?(chr_map = identity) ?(increment_lo_hi = (1, 0)) ic =
  ...
  let f acc (chr, s, f) =
    { exons = [ (s, f) ]; lo = s; hi = f; chr = chr_map chr; info = "" } :: acc
  in
    ...
    | Some prev ->
        {
          ...
          chr =
            (if row.Gff.chr = prev.chr then row.Gff.chr
            else
              failwithf "chromosome of transcript %s changed from %s to %s"
                transcript_name prev.chr row.Gff.chr ());
          info =
            (assert (transcript_name = prev.info);
             transcript_name);
        }
  in
  let g ans row =
    match transcript_name_of_exon row with
    | None -> ans
...
let all_probes_in (trx_lst : 'a t) (prbs : (string * int * int * 'b) list) :
    ('a * 'b array) t =
  let insert x prev = match prev with None -> [ x ] | Some l -> x :: l in
  let siimap_of_exons =
    let f acc trx =
      SIIMap.add trx.chr (trx.lo, trx.hi) (trx.exons, trx.info) acc
    in
...
let all_points_in (trx_lst : 'a t) (points : (string * int * 'b) list) :
    ('a * 'b array) t =
