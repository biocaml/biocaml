...
let parse_comma_ints msg pos s =
  ...
  let drop = function ' ' | '\n' | '\t' | '\r' | ',' -> true | _ -> false in
  String.strip ~drop s |> String.split ~on:','
...
module Transform = struct
  let string_to_item ?filename () =
    ...
    Lines.Transform.make_merge_error ~name ?filename
      ~next:(fun linebuf ->
        ...
        let rec get_line () =
          ...
          | Some line ->
              ...
              if
                String.equal line' version_line
                || is_spaces_line line'
...
let in_channel_to_item_stream ?(buffer_size = 65536) ?filename inp =
  Tfxm.in_channel_strings_to_stream ~buffer_size inp
