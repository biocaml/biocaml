...
module BP = struct
  ...
  let pack_u32_in_int64_little_endian v =
    ...
    Bytes.set buf 3
      (Char.unsafe_of_int
         Int64.(to_int_exn (bit_and 0xFFL (shift_right_logical v 24))));
    (* Now we can use [unsafe_set] for the intermediate bytes. *)
    Bytes.unsafe_set buf 1
      (Char.unsafe_of_int
         Int64.(to_int_exn (bit_and 0xFFL (shift_right_logical v 8))));
    Bytes.unsafe_set buf 2
      (Char.unsafe_of_int
         Int64.(to_int_exn (bit_and 0xFFL (shift_right_logical v 16))));
    Bytes.unsafe_to_string ~no_mutation_while_string_reachable:buf
end
...
let result_list_init n ~f =
  let rec aux i accu =
    if i < 0 then Ok accu
    else match f i with Ok y -> aux (i - 1) (y :: accu) | Error _ as e -> e
  in
  aux (n - 1) []
...
module Header = struct
  ...
  type t = { ref_seq : Sam.ref_seq array; sam_header : Sam.header }

  let to_sam h = h.sam_header

  let of_sam sam_header =
    { ref_seq = Array.of_list sam_header.Sam.ref_seqs; sam_header }
end
...
module Alignment0 = struct
    ...
    try
      Ok
        (Option.map (ref_id al) ~f:(fun id ->
             (Array.get header.ref_seq id).Sam.name))
  ...
  let cigar al =
    result_list_init
      ...
      ~f:(fun i ->
        let s32 =
          BP.unpack_signed_32_little_endian ~buf:al.cigar ~pos:(i * 4)
        in
        cigar_op_of_s32 s32)
  ...
  let seq al =
    ...
    if n = 0 then None
      ...
      for i = 0 to n - 1 do
        ...
        if (2 * i) + 1 < l_seq then
          Bytes.set r ((2 * i) + 1) (char_of_seq_code (c land 0xf))
      done;
  ...
  let qual al =
    ...
    | exception Failure _ ->
        Or_error.error "Bam.Alignement0.qual: incorrect quality score" al.qual
          sexp_of_string
  ...
  let parse_cstring buf pos =
    let rec aux i =
      if i < String.length buf then
        if Char.(String.get buf i = '\000') then return i else aux (i + 1)
  ...
  let parse_cCsSiIf buf pos typ =
    ...
    | 's' ->
        let i =
          Int64.of_int_exn (BP.unpack_signed_16_little_endian ~buf ~pos)
        in
        return (Sam.optional_field_value_i i, len)
    | 'S' ->
        let i =
          Int64.of_int_exn (BP.unpack_unsigned_16_little_endian ~buf ~pos)
        in
        return (Sam.optional_field_value_i i, len)
    | 'i' ->
  ...
  let parse_optional_field_value buf pos = function
    ...
    | 'Z' ->
        ...
        Sam.optional_field_value_Z s >>= fun value -> return (value, pos' - pos)
    | 'H' ->
        parse_cstring buf pos >>= fun (s, pos') ->
        Sam.optional_field_value_H s >>= fun value -> return (value, pos' - pos)
    | 'B' -> (
  ...
  let parse_optional_field buf pos =
    ...
    Sam.optional_field tag field_value >>= fun field -> return (field, shift + 3)
  ...
  let optional_fields al =
    tag
      (parse_optional_fields al.optional)
      ~tag:"Bam.Alignment0.optional_fields"
  ...
  let decode al header =
    ...
    Sam.alignment ?qname:(qname al) ~flags ?rname ?pos:(pos al) ?mapq:(mapq al)
      ~cigar ?rnext ?pnext:(pnext al) ?tlen:(tlen al) ?seq:(seq al) ~qual
      ~optional_fields ()
  ...
  let find_ref_id header ref_name =
    ...
    match
      Array.findi header.ref_seq ~f:(fun _ rs ->
          String.(rs.Sam.name = ref_name))
    with
    | Some (i, _) -> Ok i
  ...
  let sizeof al =
    let l_seq = l_seq al in
    (8 * 4) + String.length al.read_name + 1 (* NULL terminated string *)
    + String.length al.cigar
    + ((l_seq + 1) / 2)
    + l_seq + String.length al.optional
  ...
  let string_of_optional_fields opt_fields =
    let field_value_encoding = function
      ...
      | `i i ->
          ...
          if int64_fits_u32 i then Ok ('I', BP.pack_u32_in_int64_little_endian i)
          else if int64_fits_s32 i then
            Ok ('i', BP.pack_signed_32_little_endian (Int32.of_int64_exn i))
          else
            error "Sam integer cannot be encoded in BAM format" i
              Int64.sexp_of_t
      | `H s ->
          let r = ref [] in
    ...
    |> Or_error.all >>| String.concat ~sep:""
  ...
  let encode al header =
    ...
    encode_bin_mq_nl ~bin ~mapq ~l_read_name >>= fun bin_mq_nl ->
    let flags = (al.Sam.flags :> int) in
    let n_cigar_ops = List.length al.Sam.cigar in
    encode_flag_nc ~flags ~n_cigar_ops >>= fun flag_nc ->
    (match al.Sam.rnext with
...
let read_one_reference_information iz =
  try
    ...
    check (l_name > 0)
      "Incorrect encoding of reference sequence name in BAM header"
    >>= fun () ->
  ...
  with End_of_file ->
    error_string "EOF while reading BAM reference information"
...
let read_alignment_aux iz block_size =
  try
    ...
    let remaining =
      block_size - 32 - l_read_name - (4 * n_cigar_ops)
      - ((l_seq + 1) / 2)
      - l_seq
    in
...
let write_plain_SAM_header h oz =
  ...
  List.iter h.ref_seqs ~f:(fun x -> add_line (print_ref_seq x));
  List.iter h.read_groups ~f:(fun x -> add_line (print_read_group x));
  List.iter h.programs ~f:(fun x -> add_line (print_program x));
  List.iter h.comments ~f:(fun x ->
      Buffer.add_string buf "@CO\t";
      add_line x);
  List.iter h.others ~f:(fun x -> add_line (print_other x));
...
let write h xs oc =
  ...
  let xs =
    Stream.map xs ~f:(fun al ->
        match Alignment0.encode al h with Ok r -> r | Error e -> raise (M.E e))
  in
  try
    write0 h xs oc;
    Ok ()
