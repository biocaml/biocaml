  ...
  let find tbl (x, y) =
    ...
    | Caml.Not_found ->
      failwith
        (Msg.err
           (sprintf
              "CEL file does not have values for probe at position x = %d, y = %d"
              x
              y))
  ;;
end
...
module Parser = struct
  ...
  let intensity_row s =
    ...
    | [ xcoord; ycoord; mean; stdv; npixels ] ->
      ...
      ; idata = { mean = to_float mean; stdv = to_float stdv; npixels = to_int npixels }
      }
  ...
  let intensity_section lines =
    ...
    let lines =
      CFStream.Stream.take_while
        ...
        lines
    in
    let lines = CFStream.Stream.map ~f:intensity_row lines in
    let ans = CFStream.Stream.to_list lines in
    ...
    let of_channel cin =
      ...
      try
        CFStream.Stream.drop_while
          ...
          lines;
        if CFStream.Stream.is_empty lines
        then failwith (isection_name ^ " section not found");
        intensity_section
          (CFStream.Stream.map lines ~f:(fun (x : Lines.item) -> (x :> string)))
      with
...
let of_file_opt file =
  try Some (of_file file) with
  | Bad _ -> None
;;
