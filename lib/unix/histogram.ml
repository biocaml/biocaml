...
let make cmp bins =
  let rec is_ordered l =
    ...
    | _ :: [] -> true
    | x1 :: x2 :: l -> (
        match cmp x1 x2 with x when x < 0 -> is_ordered (x2 :: l) | _ -> false)
  in
...
let bin_exn hist k =
  ...
  else (hist.bin_limits.(k), hist.bin_limits.(k + 1))
...
let find_bin_index hist x =
  ...
  let () =
    for j = 0 to Array.length hist.bin_limits - 2 do
      ...
      if (cmp_lo = 1 || cmp_lo = 0) && cmp_hi = -1 then i := j
    done
  in
...
let reset hist =
  { hist with counts = Array.create ~len:(Array.length hist.counts) 0. }
...
let make_uniform min max n =
  if Float.( >= ) min max then
    Error
      (sprintf "minimum %.3f must be strictly less than maximum %.3f" min max)
  else if n < 1 then Error (sprintf "cannot create histogram with %d bins" n)
  else
    let delt = (max -. min) /. Float.of_int n in
    let bins =
      Array.init (n + 1) ~f:(fun i -> min +. (delt *. Float.of_int i))
    in
    bins.(Array.length bins - 1) <- max;
    Result.of_option
      (make Stdlib.compare (Array.to_list bins))
      ~error:"not ordered"
