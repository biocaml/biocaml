type t = { lo : int; hi : int } [@@deriving compare, sexp]

let make lo hi =
  if lo <= hi then Ok { lo; hi }
  else
    error "lower bound larger than upper bound" (lo, hi) [%sexp_of: int * int]
...
let all_positional vl =
  let compare u v =
    ...
    | Some x -> x
    | None -> (
        match compare_positional u v with Some x -> x | None -> assert false)
  in
...
let max_gap_of_positional vl =
  ...
  let rec loop ans vl =
    ...
    | [] | _ :: [] -> ans
    | u :: v :: vl ->
        if before u v then loop (max ans (gap u v)) (v :: vl)
        else
          failwith
            (sprintf "ranges %s and %s not positionally comparable"
...
let find_min_range ?(init_direction = "fwd") v pred i =
  ...
  let rec loop (dir : string) ans =
    if pred ans then Some ans
      ...
      | _ ->
          invalid_arg
            (sprintf "valid directions are \"fwd\" or \"rev\" but given \"%s\""
               dir)
  in
...
let expand_assoc_list tal =
  ...
  let ans =
    Caml.Hashtbl.fold (fun key value ans -> (key, value) :: ans) ans []
  in
  ...
  let rec loop (curr : (t * int) option) ans tal =
    ...
    | (t, a) :: tal -> (
        ...
        | Some (curr_v, curr_gap) ->
            ...
            if curr_gap > max_gap then loop None (insert curr ans) tal
