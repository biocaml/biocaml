...
let make lo hi =
  if lo <= hi
  then Ok { lo; hi }
  else error "lower bound larger than upper bound" (lo, hi) [%sexp_of: int * int]
;;
...
let compare_positional u v =
  if equal u v
  then Some 0
  else if strict_before u v
  then Some (-1)
  else if strict_after u v
  then Some 1
  else None
;;
...
let compare_containment u v =
  if equal u v
  then Some 0
  else if strict_subset u v
  then Some (-1)
  else if strict_superset u v
  then Some 1
  else None
;;
...
let all_positional vl =
  let compare u v =
    match compare_containment u v with
    | Some x -> x
    | None -> (
...
let find_min_range ?(init_direction = "fwd") v pred i =
  ...
  let rec loop (dir : string) ans =
    ...
    then None
      ...
      | _ ->
        invalid_arg
          (sprintf "valid directions are \"fwd\" or \"rev\" but given \"%s\"" dir))
  in
  loop init_direction { lo = i; hi = i }
;;

let expand_assoc_list tal =
  let ans = Stdlib.Hashtbl.create 100 in
  let insert (t, a) =
    for i = t.lo to t.hi do
      let prev =
        try Stdlib.Hashtbl.find ans i with
        | Stdlib.Not_found -> []
      in
      Stdlib.Hashtbl.replace ans i (a :: prev)
    done
  in
  let () = List.iter ~f:insert tal in
  let ans = Stdlib.Hashtbl.fold (fun key value ans -> (key, value) :: ans) ans [] in
...
let find_regions ?(max_gap = 0) pred tal =
  ...
  let tal =
    List.sort tal ~compare:(fun (u, _) (v, _) ->
      match compare_positional u v with
      | Some x -> x
      | None -> assert false)
  in
  ...
  let insert (curr : (t * int) option) ans =
    ...
    | Some (v, gap) ->
      ...
      if x <= y
      then { lo = x; hi = y } :: ans
      else failwithf "gap = %d, range = %s" gap (to_string v) ()
  in
  ...
  let rec loop (curr : (t * int) option) ans tal =
    ...
    | (t, a) :: tal -> (
      ...
      | Some (curr_v, curr_gap) ->
        ...
        let t, pred, tal =
          if extra_gap > 0
          then { lo = curr_v.hi + 1; hi = t.lo - 1 }, false, (t, a) :: tal
          else t, pred a, tal
        in
        let curr_v = { lo = curr_v.lo; hi = t.hi } in
        let curr_gap = if pred then 0 else size t + curr_gap in
        let curr = Some (curr_v, curr_gap) in
        if curr_gap > max_gap then loop None (insert curr ans) tal else loop curr ans tal)
  in
...
let rec make_random t =
  ...
  if lo <= hi then { lo; hi } else make_random t
;;
