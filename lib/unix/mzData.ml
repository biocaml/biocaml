...
let rec get_next_data xml =
  match Xmlm.input xml with
  | `Data s ->
      (* No to consecutive `Data are guaranteed, no concat *)
      s
  | `El_start _ ->
      skip_tag xml;
      (* ensure the corresponding close tag is read *)
      get_next_data xml
  | `El_end -> failwith "MzData.spectrums: got tag while looking for XML data"
  | _ -> get_next_data xml
...
module Precursor = struct
  ...
  let rec get_ionSelection xml p depth =
    ...
    | `El_start ((_, "cvParam"), attr) ->
        ...
        else get_ionSelection xml p depth
    | `El_start _ -> get_ionSelection xml p (depth + 1)
    | `El_end ->
        if depth = 0 then p (* </ionSelection> *)
  ...
  let rec get_precursor xml p =
    ...
    | `El_start _ ->
        skip_tag xml;
        get_precursor xml p (* <activation> *)
    | `El_end -> p (* </precursor> *)
  ...
  let rec add_list xml pl =
    ...
    | `El_start ((_, "precursor"), attr) ->
        ...
        let p =
          get_precursor xml
            { mslevel; mz = Float.nan; z = Float.nan; int = Float.nan }
        in
        add_list xml (p :: pl)
    | `El_start _ ->
        skip_tag xml;
...
let rec vec_of_binary_data xml =
  ...
  | `El_start ((_, "data"), atts) ->
      ...
      if Array1.dim v <> length then
        failwith
          (sprintf "MzData: Invalid XML: <data> expected length: %i, got: %i"
             length (Array1.dim v));
...
let rec get_spectrum xml spec depth =
  ...
  | `El_end -> if depth = 0 then spec else get_spectrum xml spec (depth - 1)
