...
module Transform = struct
  let try_skip_gzip_header_exn buffer =
    ...
    if flags land 0x02 <> 0 then (* Skip header CRC *)
      ignore_bytes 2;
    if !bytes_read > Bytes.length buffer then failwith "NOT-YET"
    else Ok !bytes_read

  let inflate_as_much_as_possible in_buffer buffer zstream zlib_write_buffer
      zlib_buffer_size format =
    let len = Bytes.length buffer in
    let try_to_output used_out =
      if used_out > 0 then
        `output
          (Ok (Bytes.To_string.sub zlib_write_buffer ~pos:0 ~len:used_out))
      else `not_ready
    in
    let finished, used_in, used_out =
      Zlib.inflate zstream buffer 0 len zlib_write_buffer 0 zlib_buffer_size
        Zlib.Z_SYNC_FLUSH
    in
    if used_in < len then
      if finished then
        ...
        | `gzip when len - used_in >= 8 ->
            ...
            if continue_after_gzip then
              Buffer.add_string in_buffer
                (Bytes.To_string.sub buffer ~pos:(used_in + 8)
                   ~len:(len - used_in - 8));
  ...
  let unzip ?(format = `raw) ?(zlib_buffer_size = Default.zlib_buffer_size) () =
    ...
    let current_state =
      ref (match format with `gzip -> `gzip_header | `raw -> `inflate)
    in
    let rec next stopped =
      ...
      | _ -> (
          ...
          | `inflate -> (
              try
                let inflation =
                  inflate_as_much_as_possible in_buffer buffered !zstream
                    zlib_write_buffer zlib_buffer_size format
                in
                ...
                | `finished_gzip out ->
                    ...
                    out
              with e -> `output (Error (`zlib (Exn.to_string e))))
          | `gzip_header -> (
              try
                ...
                | Ok bytes_read ->
                    ...
                    Buffer.add_string in_buffer
                      Bytes.To_string.(
                        sub buffered ~pos:bytes_read ~len:(len - bytes_read));
                    next stopped
                | Error e -> `output (Error e)
              with _ ->
  ...
  let zip ?(format = `raw) ?(level = Default.level)
      ?(zlib_buffer_size = Default.zlib_buffer_size) () =
    let zstream = ref (Zlib.deflate_init level false) in
    let in_buffer = Buffer.create 42 in
    let zlib_write_buffer = Bytes.create zlib_buffer_size in
    let state =
      ref (match format with `raw -> `deflating | `gzip -> `gzip_header)
    in
      ...
      | `gzip ->
          ...
          ( (fun buf used_in ->
            ...
            fun () ->
              ...
              Binary_packing.pack_signed_32 ~byte_order:`Little_endian ~pos:0
                ~buf !gzip_crc;
              Binary_packing.pack_signed_32 ~byte_order:`Little_endian ~pos:4
                ~buf !gzip_size;
              buf )
    in
    let next stopped =
      ...
      | `deflating -> (
          ...
          | 0 ->
              if stopped then
                if !this_is_the_end then `end_of_stream
                  ...
                  let _, _, used_out =
                    Zlib.deflate !zstream (bytes_empty ()) 0 0 zlib_write_buffer
                      0 zlib_buffer_size Zlib.Z_FINISH
                  in
                  let to_output =
          ...
          | len ->
              ...
              let _, used_in, used_out =
                Zlib.deflate !zstream buffered 0 len zlib_write_buffer 0
                  zlib_buffer_size (* Zlib.Z_NO_FLUSH *)
                  Zlib.Z_SYNC_FLUSH
              in
              update_crc buffered used_in;
              if used_in < len then
                Buffer.add_subbytes in_buffer buffered ~pos:used_in
