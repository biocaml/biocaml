...
let codes =
  ...
  in
  List.fold_left ~f:(fun s e -> Base.Set.add s e) ~init:Set.empty codes
;;

let is_nucleic_acid c = Base.Set.mem codes (Char.uppercase c)
let too_long = "sequence length exceeds " ^ string_of_int String.max_length
let bad_acid c = "invalid sequence element " ^ String.of_char c

let of_buffer b =
  let n = Buffer.length b in
  if n > String.max_length
  then raise_bad too_long
  else
...
let slice first last t =
  if first < 1 || last > String.length t || first > last
  then failwith "requesting invalid slice from sequence"
