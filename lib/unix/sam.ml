...
let ( >>?~ ) (x : 'a option Or_error.t) (f : 'a -> 'b Or_error.t) :
    'b option Or_error.t =
  let open Result.Monad_infix in
  x >>= function None -> Ok None | Some x -> f x >>| Option.some

(******************************************************************************)
(* Header Types                                                               *)
(******************************************************************************)
type header_item_tag = [ `HD | `SQ | `RG | `PG | `CO | `Other of string ]
[@@deriving sexp]
...
type group_order = [ `None | `Query | `Reference ] [@@deriving sexp]
...
module Flags = struct
  ...
  let of_int x =
    if 0 <= x && x <= 65535 then Ok x
    else error "flag out of range" x sexp_of_int
...
type optional_field = { tag : string; value : optional_field_value }
[@@deriving sexp]

type rnext = [ `Value of string | `Equal_to_RNAME ] [@@deriving sexp]
...
let parse_header_version s =
  let err =
    error "invalid version" (`HD, s) [%sexp_of: header_item_tag * string]
  in
...
let ref_seq ~name ~length ?assembly ?md5 ?species ?uri () =
  ...
  (if 1 <= length && length <= 2147483647 then Ok length
  else error "invalid reference sequence length" length sexp_of_int)
  >>= fun length ->
  (if
   String.length name > 0
   && String.foldi name ~init:true ~f:(fun i accum c ->
          accum
          && if i = 0 then is_name_first_char_ok c else is_name_other_char_ok c)
  then Ok name
  else error "invalid ref seq name" name sexp_of_string)
  >>= fun name -> Ok { name; length; assembly; md5; species; uri }

let read_group ~id ?seq_center ?description ?run_date ?flow_order ?key_seq
    ?library ?program ?predicted_median_insert_size ?platform ?platform_unit
    ?sample () =
  (match run_date with
  | None -> Ok None
  | Some run_date -> (
      ...
      with _ -> (
        ...
        with _ -> error "invalid run date/time" run_date sexp_of_string)))
  >>= fun run_date ->
  ...
  | Some x ->
      if
        String.for_all x ~f:(function
          ...
          | _ -> false)
      then Ok flow_order
      else error "invalid flow order" x sexp_of_string)
  >>| fun flow_order ->
  {
    id;
    seq_center;
...
let header ?version ?sort_order ?group_order ?(ref_seqs = [])
    ?(read_groups = []) ?(programs = []) ?(comments = []) ?(others = []) () =
  [
    (match version with
    | None -> None
    | Some x -> (
        match parse_header_version x with Error e -> Some e | Ok _ -> None));
    (if Option.is_some sort_order && Poly.(version = None) then
     Some
       (Error.create "sort order cannot be defined without version"
          (sort_order, version) [%sexp_of: sort_order option * string option])
    else None);
...
let parse_header_item_tag s =
  ...
  | Some "CO" -> Ok `CO
  | Some x ->
      if String.length x = 2 && String.for_all x ~f:is_letter then Ok (`Other x)
      else error "invalid header item tag" s sexp_of_string
...
let parse_tag_value s =
  let parse_tag s =
    if
      ...
      | _ -> false
    then Ok s
    else error "invalid tag" s sexp_of_string
  in
  let parse_value tag s =
    ...
    then Ok s
    else error "tag has invalid value" (tag, s) [%sexp_of: string * string]
  in
  match String.lsplit2 s ~on:':' with
  | None -> error "tag-value not colon separated" s sexp_of_string
  | Some (tag, value) ->
      parse_tag tag >>= fun tag ->
      parse_value tag value >>= fun value -> Ok (tag, value)
...
let assert_tags header_item_tag tvl tags =
  ...
  if Set.length unexpected_tags = 0 then Ok ()
  else
    error "unexpected tag for given header item type"
...
let parse_header_line tvl =
  ...
  find01 `HD tvl "SO" >>?~ parse_sort_order >>= fun sort_order ->
  find01 `HD tvl "GO" >>?~ parse_group_order >>= fun group_order ->
...
let parse_ref_seq tvl =
  ...
  (try Ok (Int.of_string length)
   with _ -> error "invalid ref seq length" length sexp_of_string)
  >>= fun length ->
...
let parse_read_group tvl =
  ...
  ( find01 `RG tvl "PI" >>?~ fun predicted_median_insert_size ->
    ...
    with _ ->
      error "invalid predicted median insert size" predicted_median_insert_size
        sexp_of_string )
  >>= fun predicted_median_insert_size ->
  find01 `RG tvl "PL" >>?~ parse_platform >>= fun platform ->
  find01 `RG tvl "PU" >>= fun platform_unit ->
  find01 `RG tvl "SM" >>= fun sample ->
  assert_tags `RG tvl
    [ "ID"; "CN"; "DS"; "DT"; "FO"; "KS"; "LB"; "PG"; "PI"; "PL"; "PU"; "SM" ]
  >>= fun () ->
  read_group ~id ?seq_center ?description ?run_date ?flow_order ?key_seq
    ?library ?program ?predicted_median_insert_size ?platform ?platform_unit
    ?sample ()
...
(******************************************************************************)
let alignment ?ref_seqs ?qname ~flags ?rname ?pos ?mapq ?(cigar = []) ?rnext
    ?pnext ?tlen ?seq ?(qual = []) ?(optional_fields = []) () =
  [
    (match (ref_seqs, rname) with
    | None, _ | _, None -> None
    | Some ref_seqs, Some rname ->
        if Set.mem ref_seqs rname then None
        else
          Some
            (Error.create "RNAME not defined in any SQ line" rname
               sexp_of_string));
    ...
    | Some ref_seqs, Some (`Value rnext) ->
        if Set.mem ref_seqs rnext then None
        else
          Some
            (Error.create "RNEXT not defined in any SQ line" rnext
               sexp_of_string));
    (match (seq, qual) with
    | _, [] -> None
    ...
    | Some seq, _ ->
        ...
        if s = q then None
          ...
          Some
            (Error.create "SEQ and QUAL lengths differ" (s, q)
               [%sexp_of: int * int]));
...
let parse_int_range field lo hi s =
  ...
  try
    ...
    else error out_of_range (n, lo, hi) [%sexp_of: int * int * int]
  with _ -> error not_an_int s sexp_of_string
...
let parse_opt_string field re s =
  ...
  else match s with "*" -> Ok None | _ -> Ok (Some s)

let qname_re =
  let open Re in
  alt [ char '*'; repn (alt [ rg '!' '?'; rg 'A' '~' ]) 1 (Some 255) ]
  |> compile
...
let parse_flags s =
  ...
  with _ -> error "invalid FLAG" s sexp_of_string

let rname_re = Re.Perl.compile_pat "^\\*|[!-()+-<>-~][!-~]*$"
let parse_rname s = parse_opt_string "RNAME" rname_re s

let parse_pos s =
  parse_int_range "POS" 0 2147483647 s >>| function 0 -> None | x -> Some x

let parse_mapq s =
  parse_int_range "MAPQ" 0 255 s >>| function 255 -> None | x -> Some x
...
let cigar_op_alignment_match i =
  Or_error.(positive i >>| fun i -> `Alignment_match i)
...
let cigar_op_soft_clipping i =
  Or_error.(positive i >>| fun i -> `Soft_clipping i)

let cigar_op_hard_clipping i =
  Or_error.(positive i >>| fun i -> `Hard_clipping i)
        ...
        if Scanf.Scanning.end_of_input ch then Ok accum
          ...
          try
            ...
            let x =
              ...
              | other ->
                  Or_error.error "invalid cigar operation type" other
                    Char.sexp_of_t
            in
            Or_error.tag x ~tag:"Sam.parse_cigar: invalid cigar string"
            >>= fun x -> loop (x :: accum)
          with _ -> error "invalid cigar string" text sexp_of_string
      in
      loop [] >>| List.rev
...
let parse_rnext s =
  if not (Re.execp rnext_re s) then error "invalid RNEXT" s sexp_of_string
  else
    match s with
    | "*" -> Ok None
...
let parse_pnext s =
  parse_int_range "PNEXT" 0 2147483647 s >>| function 0 -> None | x -> Some x
...
let optional_field_value_B elt_type elts =
  ...
  if valid_args then Ok (`B (elt_type, elts))
    ...
    error "invalid value" ("B", elt_type, elts)
...
let parse_optional_field_value s =
  match String.lsplit2 s ~on:':' with
  | None -> error "missing TYPE in optional field" s sexp_of_string
  | Some (typ, value) -> (
      match typ with
      | "A" ->
...
let parse_alignment ?ref_seqs line =
  ...
  | qname :: flags :: rname :: pos :: mapq :: cigar :: rnext :: pnext :: tlen
    :: seq :: qual :: optional_fields ->
      ...
      >>= fun optional_fields ->
      alignment ?ref_seqs ?qname ~flags ?rname ?pos ?mapq ~cigar ?rnext ?pnext
...
let print_ref_seq (x : ref_seq) =
  sprintf "@SQ\tSN:%s\tLN:%d%s%s%s%s" x.name x.length
...
let print_read_group (x : read_group) =
  let s tag value =
    match value with None -> "" | Some x -> sprintf "\t%s:%s" tag x
  in
  sprintf "@RG\tID:%s%s%s%s%s%s%s%s%s%s%s%s" x.id (s "CN" x.seq_center)
    (s "DS" x.description)
    (s "DT" (Option.map x.run_date ~f:(function `Date x | `Time x -> x)))
    (s "FO" x.flow_order) (s "KS" x.key_seq) (s "LB" x.library)
    (s "PG" x.program)
    (s "PI" (Option.map x.predicted_median_insert_size ~f:Int.to_string))
    (s "PL" (Option.map x.platform ~f:print_platform))
    (s "PU" x.platform_unit) (s "SM" x.sample)

let print_program (x : program) =
  let s tag value =
    match value with None -> "" | Some x -> sprintf "\t%s:%s" tag x
  in
  sprintf "@PG\tID:%s%s%s%s%s%s" x.id (s "PN" x.name) (s "CL" x.command_line)
    (s "PP" x.previous_id) (s "DS" x.description) (s "VN" x.version)
...
let print_alignment a =
  sprintf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" (print_qname a.qname)
    (print_flags a.flags) (print_rname a.rname) (print_pos a.pos)
    (print_mapq a.mapq) (print_cigar a.cigar) (print_rnext a.rnext)
    (print_pnext a.pnext) (print_tlen a.tlen) (print_seq a.seq)
    (print_qual a.qual)
...
module MakeIO (Future : Future.S) = struct
  ...
  let read_header lines =
    ...
    | Ok ({ version; sort_order; group_order; _ } as x) ->
        ...
        header ?version ?sort_order ?group_order ~ref_seqs ~read_groups
    ...
    | Ok hdr ->
        let alignments =
          Pipe.map lines ~f:(fun line ->
              Or_error.tag_arg (parse_alignment line) "position" !pos
                Pos.sexp_of_t)
        in
        Ok (hdr, alignments)
  ...
  let write_header w (h : header) =
    ...
    | Some version ->
        write_line w
          (print_header_line
             { version; sort_order = h.sort_order; group_order = h.group_order }))
    >>= fun () ->
    Deferred.List.iter h.ref_seqs ~f:(fun x -> write_line w (print_ref_seq x))
    >>= fun () ->
    Deferred.List.iter h.read_groups ~f:(fun x ->
        write_line w (print_read_group x))
    >>= fun () ->
    Deferred.List.iter h.programs ~f:(fun x -> write_line w (print_program x))
    >>= fun () ->
    Deferred.List.iter h.comments ~f:(fun x ->
        write w "@CO\t" >>= fun () -> write_line w x)
    >>= fun () ->
    Deferred.List.iter h.others ~f:(fun x -> write_line w (print_other x))

  let write w ?(header = empty_header) alignments =
    write_header w header >>= fun () ->
    Pipe.iter alignments ~f:(fun a -> Writer.write_line w (print_alignment a))
