...
module Flags = struct
  ...
  let of_int x =
    if 0 <= x && x <= 65535 then Ok x else error "flag out of range" x sexp_of_int
  ;;
...
let parse_header_version s =
  let err = error "invalid version" (`HD, s) [%sexp_of: header_item_tag * string] in
  ...
  (match run_date with
   ...
   | Some run_date -> (
     ...
     | _ -> (
       ...
       | _ -> error "invalid run date/time" run_date sexp_of_string)))
  >>= fun run_date ->
...
let parse_ref_seq tvl =
  ...
  (try Ok (Int.of_string length) with
   | _ -> error "invalid ref seq length" length sexp_of_string)
  >>= fun length ->
  find01 `SQ tvl "AS"
  >>= fun assembly ->
  find01 `SQ tvl "M5"
...
let parse_int_range field lo hi s =
  ...
  with
  | _ -> error not_an_int s sexp_of_string
;;
...
let parse_opt_string field re s =
  ...
  then error (sprintf "invalid %s" field) s sexp_of_string
  else (
    match s with
    | "*" -> Ok None
    | _ -> Ok (Some s))
...
let parse_flags s =
  ...
  | _ -> error "invalid FLAG" s sexp_of_string
;;
...
let parse_cigar text =
  ...
  | _ ->
    ...
    let rec loop accum =
      if Scanf.Scanning.end_of_input ch
      then Ok accum
      else (
        try
          ...
          >>= fun x -> loop (x :: accum)
        with
        | _ -> error "invalid cigar string" text sexp_of_string)
    in
    loop [] >>| List.rev
;;
...
let parse_rnext s =
  ...
  then error "invalid RNEXT" s sexp_of_string
  else (
    match s with
    | "*" -> Ok None
...
let parse_qual s =
  ...
  | "*" -> Ok []
  | _ -> String.to_list s |> Result.List.map ~f:(Phred_score.of_char ~offset:`Offset33)
;;
...
let print_qual = function
  | [] -> "*"
  | quals ->
    List.map quals ~f:(fun x -> ok_exn (Phred_score.to_char ~offset:`Offset33 x))
    |> String.of_char_list
;;
...
module MakeIO (Future : Future.S) = struct
  ...
  let write_header w (h : header) =
    ...
    >>= fun () ->
    Deferred.List.iter h.read_groups ~f:(fun x -> write_line w (print_read_group x))
    >>= fun () ->
    ...
    >>= fun () -> Deferred.List.iter h.others ~f:(fun x -> write_line w (print_other x))
  ;;
