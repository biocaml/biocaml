...
let transpose a =
  ...
  if Array.length a = 0 || Array.length a.(0) = 0
  then [||]
  else (
...
let odd x = x mod 2 <> 0
let min a = Option.value_exn ~message:"Math.min: empty" (Array.reduce a ~f:Poly.min)
let max a = Option.value_exn ~message:"Math.max: empty" (Array.reduce a ~f:Poly.max)
...
let range step first last =
  ...
  let n = ((last -. first) /. step |> Float.abs |> Float.round_up |> Int.of_float) + 1 in
  ...
  if comp a.(n - 1) last then a else Array.sub a ~pos:0 ~len:(n - 1)
;;
...
let rms a = Array.map ~f:(fun x -> x *. x) a |> mean |> sqrt
...
let quantile_normalization aa =
  ...
  if Array.length aa = 0 || Array.length aa.(0) = 0 || Array.length aa.(0) = 1
  then Array.copy aa
  else (
    ...
    let avg i =
      Array.fold ~f:(fun sum expt -> snd expt.(i) +. sum) ~init:0.0 aa /. num_expts
    in
...
let histogram (type t) ?(cmp = Stdlib.compare) arr =
  let module M = struct
    include Map.Make (struct
        type t_ = t
        (* required only because OCaml doesn't have type non-rec definitions *)
  ...
  let f (mp : int M.t) (a : t) =
    match Map.find mp a with
    | Some e -> Map.set mp ~key:a ~data:(e + 1)
    | None -> Map.set mp ~key:a ~data:1
  in
  let mp = Array.fold ~f ~init:M.empty arr in
  let ans = Map.fold ~f:(fun ~key ~data ans -> (key, data) :: ans) mp ~init:[] in
...
let pearson (a1 : float array) (a2 : float array) =
  ...
  let f acc e1 e2 = ((e1 -. a1avg) /. a1sd *. ((e2 -. a2avg) /. a2sd)) +. acc in
...
let rank arr =
  ...
  let f (prev, il, ans) (x, i) =
    (* prev is the value that was equal *)
    let count = List.length il in
    (* il is list of original indices in
       reverse for items that were equal *)
    if count = 0
    then
      (* ans is list of ranks and original index pairs
         in reverse *)
      x, [ i ], ans
    else if Poly.(x = prev)
    then x, i :: il, ans
    else x, [ i ], g prev il ans
  in
...
let ltqnorm p =
  ...
  *)
  ...
  then
    raise
      (ValueError
         ("Argument to ltqnorm " ^ Float.to_string p ^ " must be in open interval (0,1)"))
  else (
    ...
    then (
      ...
      f q (* Rational approximation for upper region: *))
    else if Float.(phigh < p)
    then (
      let q = sqrt (-2. *. log (1. -. p)) in
      f q (* Rational approximation for central region: *))
    else (
      let q = p -. 0.5 in
      let r = q *. q in
...
let find_regions ?(max_gap = 0) pred a =
  if max_gap < 0
  then failwith ("max gap must be non-negative but is " ^ string_of_int max_gap);
  ...
  let rec loop i start_index currGap =
    ...
    then if start_index >= 0 then loop (i + 1) start_index 0 else loop (i + 1) i 0
    else if currGap >= max_gap
    then (
      add_region i start_index currGap;
      loop (i + 1) (-1) (currGap + 1))
