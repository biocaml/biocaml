...
let column m i =
  if Array.for_all ~f:(fun row -> i < Array.length row) m then
    Array.init (Array.length m) ~f:(fun j -> m.(j).(i))
...
let prange add step lo hi =
  let rec f acc x =
    if Poly.(x > hi) then List.rev acc
    else
...
let range step first last =
  ...
  let n =
    ((last -. first) /. step |> Float.abs |> Float.round_up |> Int.of_float) + 1
  in
  let a = Array.create ~len:n 0.0 in
  let op, comp =
    Float.(if first <= last then (( +. ), ( <= )) else (( -. ), ( >= )))
  in
...
let quantile_normalization aa =
  ...
  then Array.copy aa
    ...
    let aa =
      Array.map ~f:(Array.mapi ~f:(fun i (idx, _) -> (idx, norms.(i)))) aa
    in
  ...
  let f (prev, il, ans) (x, i) =
    ...
    if count = 0 then
      (* ans is list of ranks and original index pairs
         in reverse *)
      (x, [ i ], ans)
    else if Poly.(x = prev) then (x, i :: il, ans)
...
*)
  if Float.(p <= 0. || p >= 1.) then
    raise
      (ValueError
         ("Argument to ltqnorm " ^ Float.to_string p
...
let find_regions ?(max_gap = 0) pred a =
  if max_gap < 0 then
    failwith ("max gap must be non-negative but is " ^ string_of_int max_gap);
  ...
  let rec loop i start_index currGap =
    if i = size then add_region i start_index currGap
    else if pred a.(i) then
      if start_index >= 0 then loop (i + 1) start_index 0 else loop (i + 1) i 0
    else if currGap >= max_gap then (
...
let find_min_window ?(init_direction = "fwd") a pred i =
  ...
  if size < 1 then [||]
    ...
    | None -> [||]
    | Some ans ->
        Array.sub a ~pos:ans.Range.lo ~len:(ans.Range.hi - ans.Range.lo + 1)
