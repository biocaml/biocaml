...
module Parser = struct
  let header (s : string) : string list =
    ...
    if Poly.(sl = col_names) then sl else raise_bad "incorrectly formatted header"
  ;;
  ...
  let row ~chr_map (s : string) : row =
    ...
    | [ pmcx; pmcy; mmcx; mmcy; org_ver_chr; pos; seq ] ->
      let org, ver_chr =
        ...
        | [ o; ver_chr ] -> o, ver_chr
        | _ -> raise_bad "expecting exactly one colon in Seq column"
      in
      let ver, chr =
        match String.split ver_chr ~on:';' with
        | [ v; c ] -> v, chr_map c
        | _ -> raise_bad "expecting exactly one semicolon in Seq column"
      in
      ...
      ; probe =
          ...
          ; sequence =
              (try Seq.of_string seq with
               | Seq.Bad m -> raise_bad m)
          }
      }
