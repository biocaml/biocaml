...
module Illumina = struct
  ...
  let tile_of_string s =
    ...
    if String.length s <> 4 || not (String.for_all s ~f:Char.is_digit) then
      ...
      >>= fun swath ->
      ( String.(sub s ~pos:2 ~len:(length s - 2)) |> fun x ->
        (try Ok (Int.of_string x)
         with Failure _ -> error "tile number not an int" s sexp_of_string)
        |> function
        | Error _ as e -> e
        | Ok x ->
            if x <= 0 then error "invalid tile number" x sexp_of_int else Ok x
      )
...
let name_of_line ?(pos = Pos.unknown) line =
  ...
  if n = 0 || Char.(line.[0] <> '@') then
    error "invalid name" (pos, line) [%sexp_of: Pos.t * string]
...
let comment_of_line ?(pos = Pos.unknown) line =
  ...
  let n = String.length line in
  if n = 0 || Char.(line.[0] <> '+') then
    error "invalid comment" (pos, line) [%sexp_of: Pos.t * string]
  else Ok (String.sub line ~pos:1 ~len:(n - 1))
...
let qualities_of_line ?(pos = Pos.unknown) ?sequence line =
  ...
  | Some sequence ->
      ...
      let n = String.length line in
      if m <> n then
        error "length of sequence and qualities differ" (pos, sequence, line)
...
module MakeIO (Future : Future.S) = struct
            ...
            | `Ok line -> (
                ...
                | `Ok line -> (
                    ...
                    | Ok comment -> (
                        ...
                        | `Eof ->
                            return
                              (`Ok (Error (Error.of_string "incomplete input")))
                        | `Ok line -> (
                            match
                              qualities_of_line ~sequence
                                (Line.of_string_unsafe line)
                            with
                            | Error _ as e -> return (`Ok e)
                            | Ok qualities ->
                                return
                                  (`Ok
                                    (Ok { name; sequence; comment; qualities }))
                            ))))))
  ...
  let write_item (w : Writer.t) (x : item) : unit Deferred.t =
    ...
    write_char w '+' >>= fun () ->
    write_line w x.comment >>= fun () -> write_line w x.qualities
