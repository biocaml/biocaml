...
module Illumina = struct
  ...
  let tile_of_string s =
    ...
    then error "invalid tile" s sexp_of_string
    else
      (match s.[0] with
       | '1' -> Ok `Top
       | '2' -> Ok `Bottom
      ...
      |> (fun x ->
           (try Ok (Int.of_string x) with
            | Failure _ -> error "tile number not an int" s sexp_of_string)
           |> function
           | Error _ as e -> e
           | Ok x -> if x <= 0 then error "invalid tile number" x sexp_of_int else Ok x)
  ...
  let sequence_id_of_string s =
    ...
    let i name value =
      ...
      | Failure _ -> error (sprintf "%s not an int" name) value sexp_of_string
    in
    let b name value =
      match value with
      | "Y" -> Ok true
...
(* Parsing                                                                    *)
(******************************************************************************)
let name_of_line ?(pos = Biocaml.Pos.unknown) line =
  let line = (line : Biocaml.Line.t :> string) in
  let n = String.length line in
  if n = 0 || Char.(line.[0] <> '@')
  then error "invalid name" (pos, line) [%sexp_of: Biocaml.Pos.t * string]
  else Ok (String.sub line ~pos:1 ~len:(n - 1))
;;

let sequence_of_line ?pos:_ line = (line : Biocaml.Line.t :> string)

let comment_of_line ?(pos = Biocaml.Pos.unknown) line =
  let line = (line : Biocaml.Line.t :> string) in
  let n = String.length line in
  if n = 0 || Char.(line.[0] <> '+')
  then error "invalid comment" (pos, line) [%sexp_of: Biocaml.Pos.t * string]
  else Ok (String.sub line ~pos:1 ~len:(n - 1))
;;

let qualities_of_line ?(pos = Biocaml.Pos.unknown) ?sequence line =
  let line = (line : Biocaml.Line.t :> string) in
  match sequence with
  | None -> Ok line
  | Some sequence ->
    ...
    then
      error
        ...
        (pos, sequence, line)
        [%sexp_of: Biocaml.Pos.t * string * string]
    else Ok line
;;
...
module MakeIO (Future : Future.S) = struct
  ...
  let read_item ic : item Or_error.t Reader.Read_result.t Deferred.t =
    ...
    | `Eof -> return `Eof
    | `Ok line -> (
      match name_of_line (Biocaml.Line.of_string_unsafe line) with
      ...
      | Ok name -> (
        ...
        | `Ok line -> (
          let sequence = sequence_of_line (Biocaml.Line.of_string_unsafe line) in
          Reader.read_line ic
          >>= function
          | `Eof -> return (`Ok (Error (Error.of_string "incomplete input")))
          | `Ok line -> (
            match comment_of_line (Biocaml.Line.of_string_unsafe line) with
            | Error _ as e -> return (`Ok e)
            | Ok comment -> (
              Reader.read_line ic
              >>= function
              ...
              | `Ok line -> (
                match
                  qualities_of_line ~sequence (Biocaml.Line.of_string_unsafe line)
                with
                | Error _ as e -> return (`Ok e)
                | Ok qualities -> return (`Ok (Ok { name; sequence; comment; qualities }))
                ))))))
  ;;
