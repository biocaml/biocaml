...
let background_of_sequence seq pc =
  let counts = Caml.Array.make 4 0 and n = ref 0 in
...
let make mat bg =
  ...
  map mat ~f:(fun p ->
      ...
      let r =
        mapi p ~f:(fun i x -> log ((float x +. bg.(i)) /. float n /. bg.(i)))
      in
      let n_case =
...
let gen_scan f init mat seq tol =
  let r = ref init and n = String.length seq and m = Array.length mat in
  let seq = Array.init n ~f:(fun i -> int_of_char seq.[i]) in
  for i = n - m downto 0 do
    let score = ref 0. in
    for j = 0 to m - 1 do
      score :=
        !score +. Array.(unsafe_get (unsafe_get mat j) (unsafe_get seq (i + j)))
    done;
...
let best_hit mat seq =
  let ((pos, _) as r) =
    gen_scan
      ...
      (-1, Float.neg_infinity) mat seq Float.neg_infinity
  in
  if pos < 0 then
    raise (Invalid_argument "Pwm.best_hit: sequence shorter than the matrix")
  else r

external stub_fast_scan : t -> int array -> float -> (int * float) list
  = "biocaml_pwm_scan"
