...
let left_end = function Empty -> Int.max_value | Node n -> n.left_end
let right_end = function Empty -> Int.min_value | Node n -> n.right_end
  ...
  Node
    {
      ...
      right_end =
        (let lre = right_end l and rre = right_end r in
         if hi > lre then if hi > rre then hi else rre
         else if lre > rre then lre
         else rre);
    }
...
let rec cons_iter s t =
  match s with Empty -> t | Node n -> cons_iter n.left (C (n, n.right, t))

let rec rev_cons_iter s t =
  match s with Empty -> t | Node n -> rev_cons_iter n.right (C (n, n.left, t))
...
let rec check_height_integrity = function
  ...
  | Node n ->
      let h =
        max (check_height_integrity n.left) (check_height_integrity n.right) + 1
      in
      assert (h = n.height);
    ...
    | h :: t -> (
        ...
        | Node n ->
            if interval_overlap lo hi n.left_end n.right_end then
              ...
              if interval_overlap lo hi n.lo n.hi then Some (node_contents n, t)
              else loop t
            else loop t)
  in
    ...
    | h :: t -> (
        ...
        | Node n ->
            if interval_overlap low high n.left_end n.right_end then (
              let t = n.left :: n.right :: t in
              if interval_overlap low high n.lo n.hi then
                res := add !res ~low:n.lo ~high:n.hi ~data:n.elt;
              loop t)
            else loop t)
  in
  loop [ t ];
