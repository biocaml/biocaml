...
let update_gen s flags =
  ...
  if not_passing_quality_controls flags then { s with total }
  else
    let qc_pass = s.qc_pass + 1 in
    let single_reads =
      incr_if (not (has_multiple_segments flags)) s.single_reads
    in
    let pair_witness = has_multiple_segments flags && first_segment flags in
    let read_pairs = incr_if pair_witness s.read_pairs in
    let main_mapping =
      not
        (segment_unmapped flags || secondary_alignment flags
...
let update s al = update_gen s al.Sam.flags

let update0 s al =
  let open Or_error.Monad_infix in
  Bam.Alignment0.flags al >>| update_gen s

module Fragment_length_histogram = struct
  type t = { min_mapq : int; counts : int Accu.Counter.t }

  let create ?(min_mapq = 0) () = { min_mapq; counts = Accu.Counter.create () }
  ...
  let with_mapped_pair ~min_mapq al f =
    ...
    let each_segment_properly_aligned =
      Sam.Flags.each_segment_properly_aligned fl
    in
  ...
  let update0 { min_mapq; counts } al =
    with_mapped_pair ~min_mapq al (fun () ->
        let length =
          match Bam.Alignment0.tlen al with
          | Some l -> Int.abs l
          | _ ->
              assert
                false (* both reads are mapped so there should be a length *)
        in
...
module Chr_histogram = struct
  ...
  let create ?(min_mapq = 0) bam_header =
    { min_mapq; bam_header; counts = Accu.Counter.create () }
