...
let tree_of_string str = in_tree (Xmlm.make_input (`String (0, str)))

let rec string_of_tree = function
  | E (tag, _, children) ->
      sprintf "<%s>%s<%s/>" tag
        (String.concat ~sep:"" (List.map ~f:string_of_tree children))
        tag
  | D s -> s

let attr k = function
  | E (_, attrs, _) ->
      List.find_map attrs ~f:(fun ((_, k'), value) ->
          if String.(k = k') then Some value else None)
  | _ -> None
...
let leaf_exn f k x =
  ...
  with Caml.Not_found -> invalid_arg (sprintf "Entrez.leaf: no %s child" k)
...
let tag_of_tree = function E (tag, _, _) -> Some tag | D _ -> None
...
let echild_exn k = function
  | E (_, _, children) -> (
      ...
      with Caml.Not_found ->
        ...
        let msg =
          sprintf "child: looked for %s but only got %s children" k
            (String.concat ~sep:"," tags)
        in
        raise (Invalid_argument msg))
  | D _ ->
      let msg =
        sprintf "child: looked for %s tag but only got a PCDATA node" k
      in
      raise (Invalid_argument msg)
...
let map_echildren ?tag f x =
  fold_echildren ?tag (fun x accu -> f x :: accu) x []
...
let esearch_url ?retstart ?retmax ?rettype ?field ?datetype ?reldate ?mindate
    ?maxdate database query =
  search_base_url ^ "?"
  ^ parameters
      Option.
        [
          ...
          map
            ~f:(function
              | `uilist -> ("rettype", "uilist") | `count -> ("rettype", "count"))
            rettype;
...
let esearch_answer_of_string str =
  tree_of_string str |> snd |> esearch_answer_of_tree

let summary_base_url =
  "http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi"

let esummary_url ?retstart ?retmax db ids =
  if List.length ids > 200 then
    raise
      (Invalid_argument
         "Entrez.esummary_url: cannot fetch more than 200 summaries");
  summary_base_url ^ "?"
  ^ parameters
      Option.
        [
...
let efetch_url ?rettype ?retmode ?retstart ?retmax ?strand ?seq_start ?seq_stop
    db ids =
  if List.length ids > 200 then
    raise
      (Invalid_argument "Entrez.efetch_url: cannot fetch more than 200 records");
  fetch_base_url ^ "?"
  ^ parameters
      Option.
        [
          ...
          map
            ~f:(fun s ->
              ("strand", match s with `plus -> "1" | `minus -> "2"))
            strand;
...
module Make (F : Fetch) = struct
  ...
  module Object_id = struct
    ...
    let to_string = function `int i -> string_of_int i | `string s -> s
    ...
    let of_xml x =
      ...
      with _ -> (
        ...
        with _ ->
          invalid_arg
            (sprintf "Entrez.Make.Object_id.of_xml: %s" (string_of_tree x)))
  end
    ...
    let of_xml x =
      {
        ...
        tag =
          Object_id.of_xml
            (x |> echild_exn "Dbtag_tag" |> echild_exn "Object-id");
      }
  end
  ...
  module Gene_ref = struct
    ...
    let of_xml t =
      let t = echild_exn "Gene-ref" t in
      {
        ...
        db =
          Option.value_map (echild "Gene-ref_db" t) ~default:[]
  ...
  module PubmedSummary = struct
    ...
    let parse_document_summary x =
      let article_ids = parse_article_ids (echild_exn "ArticleIds" x) in
      {
        pmid =
          int_of_string
            (List.Assoc.find_exn ~equal:String.equal article_ids "pubmed");
    ...
    let parse_eSummaryResult x =
      map_echildren ~tag:"DocumentSummary" parse_document_summary
  ...
  module Pubmed = struct
    ...
    let parse_pubmed_article_set_element x =
      match tag_of_tree x with
      | Some "PubmedArticle" ->
      ...
      | Some t ->
          failwith
            (sprintf "Unexpected %s tag while parsing PubmedArticleSet element"
               t)
      | None -> None
