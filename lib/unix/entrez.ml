...
(* ********************************* *)
type tree =
  | E of string * Xmlm.attribute list * tree list
  | D of string
...
let leaf_exn f k x =
  try
    ...
    | D _ -> raise Stdlib.Not_found
  with
  | Stdlib.Not_found -> invalid_arg (sprintf "Entrez.leaf: no %s child" k)
;;
...
let echild_exn k = function
  | E (_, _, children) -> (
    ...
    | Stdlib.Not_found ->
      ...
      let msg =
        sprintf
          "child: looked for %s but only got %s children"
          k
...
let esearch_answer_of_string str = tree_of_string str |> snd |> esearch_answer_of_tree
...
let esummary_url ?retstart ?retmax db ids =
  ...
  then
    raise (Invalid_argument "Entrez.esummary_url: cannot fetch more than 200 summaries");
  summary_base_url
  ^ "?"
  ^ parameters
      Option.
...
module Make (F : Fetch) = struct
  ...
  let search_and_fetch database of_xml query =
    ...
    fetch query_url esearch_answer_of_string
    >>= fun answer ->
  ...
  module Object_id = struct
    ...
    let of_xml x =
      try `int (ileaf_exn "Object-id_id" x) with
      | _ -> (
        try `string (sleaf_exn "Object-id_str" x) with
        | _ -> invalid_arg (sprintf "Entrez.Make.Object_id.of_xml: %s" (string_of_tree x))
        )
    ;;
  end
  ...
  module PubmedSummary = struct
    ...
    let parse_document_summary x =
      ...
      { pmid =
          int_of_string (List.Assoc.find_exn ~equal:String.equal article_ids "pubmed")
  ...
  module Gene = struct
    ...
    let parse_entrez_gene_set = function
      | E ("Entrezgene-Set", _, children) -> List.filter_map ~f:parse_entrez_gene children
      | _ -> assert false
    ;;
