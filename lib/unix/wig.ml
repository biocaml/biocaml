...
module Tags = struct
  type t = { allow_empty_lines : bool; sharp_comments : bool } [@@deriving sexp]
...
module Transform = struct
  ...
  let explode_key_value loc s =
    ...
    with Caml.Not_found -> Error (`cannot_parse_key_values (loc, s))
  ...
  let rec next ~tags p =
    ...
    | Some l -> (
        ...
        | [ one_value ] -> (
            ...
            with _ ->
              `output (Error (`wrong_fixed_step_value (current_position p, l))))
        | [ fst_val; snd_val ] -> (
            try
              `output
                (Ok
                   (`variable_step_value
                     (Int.of_string fst_val, Float.of_string snd_val)))
            with _ ->
              `output
                (Error (`wrong_variable_step_value (current_position p, l))))
        | [ chr; b; e; v ] -> (
            try
              `output
                (Ok
            ...
            with _ ->
              `output (Error (`wrong_bed_graph_value (current_position p, l))))
    ...
    Tfxm.make ~name:"wig_to_variable_step" ()
      ~feed:(function
        ...
        | `variable_step_value (pos, v) -> (
            ...
            | _ ->
                Queue.enqueue queue
                  (`output (Error `not_in_variable_step_state)))
        | `fixed_step_state_change (chrom, start, step, span) ->
        ...
        | `fixed_step_value v -> (
            ...
            | Some (`fixed (chrom, start, step, span, current)) ->
                ...
                Queue.enqueue queue (`output (Ok (chrom, pos, stop, v)));
                current_state :=
                  Some (`fixed (chrom, start, step, span, current + 1))
            | _ ->
