module Safe = struct
  type error = [ `invalid_int of string | `invalid_float of string ]

  let int_of_string s =
    try Ok (Int.of_string s) with _ -> Error (`invalid_int s)

  let float_of_string s =
    try Ok (Float.of_string s) with _ -> Error (`invalid_float s)
end
...
type vcf_format_type =
  [ `integer_value | `float_value | `character_value | `string_value ]
...
and reserved_alt =
  Hashtbl.Poly.of_alist_exn
    [
      ...
      ( "INS:ME",
        Alt "ME Insertion of a mobile element relative to the reference" );
    ]
...
let string_to_vcfr_ids { vcfm_id_cache; _ } s =
  ...
  | chunks ->
      ...
      if List.is_empty duplicate_ids then Ok chunks
  ...
  | chunks ->
      let res =
        List.map chunks ~f:(fun chunk ->
            ...
            match
              Char.(chunk.[0] = '<' && chunk.[n - 1] = '>', is_valid_dna chunk)
            with
            | true, _ ->
...
let list_to_vcfr_samples { vcfm_format; vcfm_samples; _ } chunks =
  ...
  let go sample_keys id raw_sample =
    ...
    if List.(length sample_keys <> length sample_chunks) then
      Error (`malformed_sample raw_sample)
    else
      let res =
      ...
      :: "INFO" :: rest -> (
        ...
        | "FORMAT" :: (_ :: _ as samples) | ([] as samples) ->
            ...
            (* Note(superbobry): merge parsed INFO and FORMAT entries with
               ...
               used. *)
            let vcfm_info =
              merge_with_reserved reserved_info vcfm_info
                ~c:(fun n t description -> Info (n, t, description))
            and vcfm_format =
              merge_with_reserved reserved_format vcfm_format
                ~c:(fun n t description -> Format (n, t, description))
            and vcfm_header, vcfm_samples =
              if List.is_empty samples then (chunks, samples)
              else List.split_n chunks List.(length chunks - length samples)
            in
            Ok
              (`complete
                { meta with vcfm_info; vcfm_format; vcfm_header; vcfm_samples })
    ...
    | Some l when String.is_prefix l ~prefix:"##" -> (
        ...
        | Some ("FILTER", v) ->
            Scanf.sscanf v "<ID=%s@,Description=%S>" (fun id description ->
                ...
                let meta =
                  { meta with vcfm_filters = (id, filter_meta) :: vcfm_filters }
                in
                Ok (`partial meta))
        | Some ("FORMAT", v) ->
            Scanf.sscanf v "<ID=%s@,Number=%s@,Type=%s@,Description=%S>"
