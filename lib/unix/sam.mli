...
type header_item_tag = private
  [< `HD | `SQ | `RG | `PG | `CO | `Other of string ]
[@@deriving sexp]
...
type optional_field = private { tag : string; value : optional_field_value }
[@@deriving sexp]

type rnext = private [< `Value of string | `Equal_to_RNAME ] [@@deriving sexp]
...
module MakeIO (Future : Future.S) : sig
  ...
  val write :
    Writer.t -> ?header:header -> alignment Pipe.Reader.t -> unit Deferred.t
...
val optional_field_value_B :
  char -> string list -> optional_field_value Or_error.t
