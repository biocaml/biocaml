...
type item = [ `comment of string | `record of record ] [@@deriving sexp]
...
module Transform = struct
  ...
  let parse_string msg pos i =
    try Ok (Scanf.sscanf i "%S " Fun.id)
    with _ -> (
  ...
  let parse_string_opt m pos i =
    ...
    match s with "." -> Ok None | s -> Ok (Some s)

  let parse_int msg pos i =
    parse_string msg pos i >>= fun s ->
    try Ok (Int.of_string s) with _ -> Error (`cannot_parse_int (pos, msg))
  ...
  let parse_attributes_version_3 position i =
    ...
    let rec loop pos acc =
      ...
      | Some equal -> (
          ...
          let pos = equal + 1 in
          match
            String.lfindi whole_thing ~pos ~f:(fun _ c -> Char.equal c ';')
          with
          | Some semicolon ->
              let delimited = String.slice whole_thing pos semicolon in
              get_csv delimited >>= fun values ->
              loop (semicolon + 1) ((tag, values) :: acc)
          | None ->
              let delimited =
                String.(sub whole_thing ~pos ~len:(length whole_thing - pos))
              in
              get_csv delimited >>= fun values -> Ok ((tag, values) :: acc))
      | None ->
          if pos >= String.length whole_thing then Ok acc
  ...
  let parse_attributes_version_2 position l =
    ...
    let parse_string i =
      try Some (Scanf.bscanf i "%S " Fun.id)
      with _ -> (
        match Scanf.bscanf i "%s " Fun.id with "" -> None | s -> Some s)
    in
    ...
    | seqname :: source :: feature :: start :: stop :: score :: strand :: phase
      :: rest ->
        let result =
          ...
          (parse_string_opt "Strand" pos strand >>= function
           | Some "+" -> Ok `plus
  ...
  let rec next ~tags p =
    ...
    | None -> `not_ready
    | Some "" ->
        if tags.Tags.allow_empty_lines then
          `output (Error (`empty_line (current_position p)))
        else next ~tags p
    | Some l
      when tags.Tags.sharp_comments && String.(is_prefix (strip l) ~prefix:"#")
      ->
...
let in_channel_to_item_stream ?(buffer_size = 65536) ?filename
    ?(tags = Tags.default) inp =
