...
module Transform = struct
  ...
  *)
    ...
    try
      ...
      | Some colon -> (
          ...
          | Some dash ->
              ...
              let stop =
                String.slice s (dash + 1) (String.length s) |> Int.of_string
              in
              Ok (`browser (`position (name, start, stop)))
          | None -> failwith "A")
      | None -> failwith "B"
    with _ -> Error (`wrong_browser_position (position, s))
  ...
  let escapable_string (s : string) ~(stop_before : char list) :
    ...
    let try_escaped s =
      try Some (Scanf.sscanf s "%S%n" (fun s n -> (s, n))) with _ -> None
    in
    ...
    | None -> (
        match
          String.lfindi s ~f:(fun _ c ->
              List.exists stop_before ~f:Char.(( = ) c))
        with
        | Some idx ->
  ...
  let parse_track stripped =
    ...
    let rec loop s acc =
      ...
      | str, _, rest ->
          Error (`wrong_key_value_format (List.rev acc, str, rest))
    in
  ...
  let rec next p =
    ...
    | Some l when String.(strip l = "browser") ->
        `output (Ok (`browser (`unknown l)))
  ...
  let string_to_string_content ?filename () =
    let name =
      sprintf "track_parser:%s" Option.(value ~default:"<>" filename)
    in
  ...
  let embed_parser ?filename =
    ...
    Tfxm.filter_compose track_parser ~destruct:(function
      ...
      | (Ok (`track _) | Ok (`browser _) | Ok (`comment _) | Error _) as n ->
          `bypass n)
  ...
  let string_to_wig ?filename () =
    ...
    Tfxm.filter_compose
      ...
      | Error _ as n -> `No n) *)
      wig_parser ~reconstruct:(function
