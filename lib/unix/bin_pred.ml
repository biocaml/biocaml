type confusion_matrix = { tp : int; tn : int; fp : int; fn : int }
[@@deriving sexp]
...
let confusion_matrix ~scores ~labels ~threshold =
  if Array.length scores <> Array.length labels then
    invalid_argf
      "Bin_pred.confusion_matrix: scores and labels have different lengths (%d \
       and %d)"
...
let performance_curve ~scores ~labels =
  ...
  if n <> Array.length labels then
    invalid_argf
      "Bin_pred.make_curve: scores and labels have different lengths (%d and \
       %d)"
  ...
  let rec loop acc current_threshold current_matrix i =
    if i = n then List.rev ((current_threshold, current_matrix) :: acc)
    else
      ...
      let new_matrix =
        if label then
          {
            current_matrix with
            tp = current_matrix.tp + 1;
            fn = current_matrix.fn - 1;
          }
        else
          {
            current_matrix with
            fp = current_matrix.fp + 1;
...
let%expect_test "performance curve 1" =
  ...
  [%expect
    "\n\
    \    ((INF ((tp 0) (tn 2) (fp 0) (fn 2))) (5.6 ((tp 1) (tn 2) (fp 0) (fn \
     1)))\n\
    \     (2.1 ((tp 2) (tn 2) (fp 0) (fn 0))) (1.2 ((tp 2) (tn 1) (fp 1) (fn \
     0)))\n\
