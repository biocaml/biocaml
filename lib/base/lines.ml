module Parser = struct
  type state =
    | Current_line of
        { n : int
        ; value : Biocaml.Line.t
        }
    | Finished of { n : int }
  (* [n] represents the number of completed lines *)

  let initial_state = Current_line { n = 0; value = Biocaml.Line.empty }
  ...
  let step st i =
    ...
    | Current_line { n; value }, None ->
      Finished { n }, if Biocaml.Line.is_empty value then [] else [ value ]
    | Current_line { n; value }, Some input -> (
      match Biocaml.Line.rightmost input with
      | None, line ->
        let value = Biocaml.Line.append value line in
        Current_line { n; value }, []
      | Some left, right -> (
        match Biocaml.Line.parse_string left with
        | [] -> assert false (* [Biocaml.Line.parse_string] never returns an empty list *)
        | h :: t ->
          ( Current_line { n = n + List.length t + 1; value = right }
          , Biocaml.Line.append value h :: t )))
  ;;
end
