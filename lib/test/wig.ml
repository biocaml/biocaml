...
let file_parser_stream file =
  ...
  let t = Wig.Transform.string_to_item ~filename () in
...
let check_error s m f =
  assert_bool
    (sprintf "check_error: %s" m)
    (match Stream.next s with Some (Error e) -> f e | _ -> false)
...
let test_parser () =
  ...
  check_output s "fixed_step_state_cha"
  ...
  check_error s "missing_start_value" (function
    | `missing_start_value (_, "fixedStep chrom=chr19  step=300 span=200") ->
        true
    | _ -> false);
