...
let test_parser () =
  ...
  test_line [ "\"big\\tC style\""; "some"; "s"; "42"; "43"; "2."; "wrong"; "2" ] (function
  ...
  let transfo =
    let tags =
      { Gff.Tags.default with Gff.Tags.version = `two; allow_empty_lines = true }
    in
    Gff.Transform.string_to_item ~tags ()
  in
  let test_line l f =
    let joined = String.concat ~sep:"\t" l in
    Tfxm.feed transfo (joined ^ "\n");
...
let test_printer () =
  ...
  let transfo =
    let tags =
      { Gff.Tags.default with Gff.Tags.version = `two; allow_empty_lines = true }
    in
    Gff.Transform.item_to_string ~tags ()
  in
  let test s item =
    Tfxm.feed transfo item;
    let res = Tfxm.next transfo in
...
let tests = "GFF" >::: [ "Parse GFF" >:: test_parser; "Print GFF" >:: test_printer ]
