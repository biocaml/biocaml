...
let compare_rows r1 r2 =
  ...
  && r1.vcfr_pos = r2.vcfr_pos && r1.vcfr_ids = r2.vcfr_ids
...
let test_parse_vcf_generic filename rows =
  ...
  List.iter rows ~f:(fun row ->
      ...
      | Some (Ok actual_row) -> assert_equal ~cmp:compare_rows row actual_row
      | Some (Error err) ->
...
let test_parse_vcf_simple () =
  test_parse_vcf_generic "vcf_02_simple.vcf"
    [
      make_row ~chrom:"20" ~pos:14370 ~ids:[ "rs6054257" ] ~ref:"G"
        ~alts:[ "A" ] ~qual:(Some 29.0) ~filter:[]
        ~info:
          [
            ("NS", [ `integer 3 ]);
            ("DP", [ `integer 14 ]);
...
let test_parse_vcf_1000g () =
  test_parse_vcf_generic "vcf_03_1000g.vcf"
    [
      make_row ~chrom:"20" ~pos:17330 ~ids:[] ~ref:"T" ~alts:[ "A" ] ~qual:None
        ~filter:[ "q10" ]
        ~info:
          [
            ("NS", [ `integer 3 ]);
        ...
        ~samples:[];
      make_row ~chrom:"20" ~pos:1110696 ~ids:[ "rs6040355" ] ~ref:"A"
        ~alts:[ "G"; "T" ] ~qual:(Some 67.0) ~filter:[]
        ~info:
          [
            ("NS", [ `integer 2 ]);
            ("DP", [ `integer 10 ]);
        ...
        ~samples:[];
      make_row ~chrom:"20" ~pos:1230237 ~ids:[] ~ref:"T" ~alts:[]
        ...
        ~samples:[];
      make_row ~chrom:"20" ~pos:1234567 ~ids:[ "microsat1" ] ~ref:"GTC"
        ~alts:[ "G"; "GTCT" ] ~qual:(Some 50.0) ~filter:[]
        ~info:
          [
            ("NS", [ `integer 3 ]);
            ("DP", [ `integer 9 ]);
...
let test_parse_vcf_alt () =
  test_parse_vcf_generic "vcf_05_alt.vcf"
    [
      make_row ~chrom:"2" ~pos:321682 ~ids:[] ~ref:"T" ~alts:[ "<DEL>" ]
        ...
        ~samples:[];
      make_row ~chrom:"2" ~pos:14477084 ~ids:[] ~ref:"C"
        ~alts:[ "<DEL:ME:ALU>" ] ~qual:(Some 12.0) ~filter:[]
        ~info:
          [
            ("IMPRECISE", [ `flag "IMPRECISE" ]);
            ("SVTYPE", [ `string "DEL" ]);
            ...
            ("SVLEN", [ `integer (-297) ]);
            ( "MEINFO",
              [ `string "AluYa5"; `string "5"; `string "307"; `string "+" ] );
...
let tests =
  ...
  >::: [
         ...
         "Parse VCF missing INFO for reserved sub-fields"
         >:: test_parse_vcf_reserved;
