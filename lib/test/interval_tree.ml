...
let test_creation () =
  for _ = 1 to 100 do
    ...
    assert_equal
      ...
      100
      ~msg:"Verify list result length"
      (List.length lres);
    assert_equal
      ~printer:string_of_int
      100
      ~msg:"Verify tree result length"
      (List.length tres);
...
let test_find_intersecting_elem1 () =
  ...
  in
  assert_equal T.(find_intersecting_elem 70163 70163 u |> Stream.to_list |> List.length) 1;
  assert_equal T.(find_intersecting_elem 65163 75163 u |> Stream.to_list |> List.length) 3
;;
