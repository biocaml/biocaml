...
module ListImpl = struct
  ...
  let elements x = x
  let to_stream x = List.sort ~compare:Poly.compare x |> Stream.of_list
  ...
  let find_intersecting_elem lo hi t =
    Stream.filter ~f:(fun (x, y, _) -> interval_overlap lo hi x y) (to_stream t)
  ...
  for _ = 1 to 100 do
    ...
    ignore
      (List.fold_left intervals ~init:T.empty ~f:(fun accu (lo, hi, _) ->
           ...
           Interval_tree.check_integrity r;
           r)
...
let test_creation () =
  for _ = 1 to 100 do
    ...
    assert_equal ~printer:string_of_int 100 ~msg:"Verify list result length"
      (List.length lres);
    assert_equal ~printer:string_of_int 100 ~msg:"Verify tree result length"
      (List.length tres);
...
let test_find_intersecting_elem1 () =
  let u =
    T.(
      add ~low:69130 ~high:69630 ~data:()
        (add ~low:69911 ~high:70411 ~data:()
           (add ~low:70501 ~high:71001 ~data:() empty)))
  in
  assert_equal
...
let test_find_intersecting_elem2 () =
  for _ = 1 to 1000 do
    ...
    and lo, hi, _ = random_interval ~ub:1000 () in
    let l =
      L.(find_intersecting_elem lo hi (of_list intervals)) |> Stream.to_set
    and t =
      T.(find_intersecting_elem lo hi (of_list intervals)) |> Stream.to_set
    in
