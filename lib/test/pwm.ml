...
let test_c_and_caml_versions_agree () =
  ...
  let mat = dr5_matrix ~seq () in
  let c_res = fast_scan mat seq (-10.) and ocaml_res = scan mat seq (-10.) in
  assert_bool "Hits number" List.(length c_res = length ocaml_res);
  assert_bool "Same positions"
  ...
  in
  assert_bool "Score no more different than eps=1e-4" Float.(eps < 1e-4)
...
let test_reverse_complement () =
  ...
  assert_bool "Reverse complement should be idempotent" (Poly.equal a a'');
  assert_bool "Wrong permutation of the first element of the matrix"
...
let test_best_hit () =
  ...
  let f s =
    let all_hits = scan m s Float.neg_infinity and _, best_hit = best_hit m s in
    let best_of_all_hits =
      List.fold_left
        ~f:(fun accu (_, s) -> Float.max accu s)
        ~init:Float.neg_infinity all_hits
    in
    Float.(best_hit = best_of_all_hits)
  in
  assert_bool "best_hit returns the best position found by scan"
