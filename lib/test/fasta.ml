...
let fasta_of_strings ~initial_state ~step xs =
  List.fold xs
    ...
    ~f:(fun accu s ->
      match accu with
      | Ok (st, items) ->
          step st (Some s) >>| fun (st', items') -> (st', items @ items')
      | e -> e)
  >>= fun (st, items) ->
  step st None >>| fun (_, final_items) -> items @ final_items

module Parser0 = struct
  type parsing_result = (Fasta.item0 list, Fasta.parser_error) result
  [@@deriving sexp]
...
module Parser = struct
  type parsing_result = (Fasta.item list, Fasta.parser_error) result
  [@@deriving sexp]
...
let tests =
  "Fasta" >::: [ "parser0" >:: Parser0.test; "parser" >:: Parser.test ]
