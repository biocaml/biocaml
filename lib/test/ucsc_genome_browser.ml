...
let test_track_line_to_string_cases =
  [
    ( "track type=bigBed name=\"bigBed Example One\" description=\"A bigBed \
       file\" \
...
let test_custom_track_url () =
  List.iter test_custom_track_url_cases
    ~f:(fun (answer, (db, position, data_url)) ->
...
let test_bigData_custom_track_url () =
  List.iter test_bigData_custom_track_url_cases
    ~f:(fun (answer, (db, position, track)) ->
      let generated_url =
        UGB.bigData_custom_track_url ~db ~position ~track ()
      in
      assert_equal ~printer:Fun.id answer generated_url)
