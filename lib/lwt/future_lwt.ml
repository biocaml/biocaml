    ...
    module List = struct
      let map ?(how = `Sequential) l ~f =
        let map =
          ...
          | `Max_concurrent_jobs _ | `Parallel -> Lwt_list.map_p
        in
        let module M = struct
      ...
      let iter ?(how = `Sequential) l ~f =
        let iter =
          ...
          | `Max_concurrent_jobs _ | `Parallel -> Lwt_list.iter_p
        in
        let module M = struct
          exception E of Error.t
...
let try_with f =
  Lwt.catch (fun () -> f () >>| fun x -> Ok x) (fun exn -> return (Error exn))
;;
...
module Pipe = struct
  ...
  let fold r ~init ~f = Lwt_stream.fold_s (fun a accum -> f accum a) r init
...
module Reader = struct
  ...
  let read_all ic read_one =
    Lwt_stream.from (fun () ->
      ...
      | `Eof -> Lwt_io.close ic >>= fun () -> Lwt.return None)
  ;;

  let lines ic = read_all ic read_line
  let contents ic = Lwt_io.read ic >>= fun ans -> Lwt_io.close ic >>= fun () -> return ans
  let file_contents file = with_file file ~f:Lwt_io.read
  let file_lines file = Lwt_io.lines_of_file file |> Lwt_stream.to_list
end
