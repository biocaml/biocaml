...
let core_split () = String.split ~on:'\n' s |> ignore
let sosa_split () = Sosa.Native_string.split ~on:(`Character '\n') s |> ignore
let container_split () = CCString.lines s |> ignore
...
let specialized_split str =
  ...
  let rec loop acc last_pos pos =
    if pos = -1
    then String.sub str ~pos:0 ~len:last_pos :: acc
    else if str.[pos] = '\n'
    then (
      let pos1 = pos + 1 in
...
let specialized_split () = specialized_split s |> ignore
