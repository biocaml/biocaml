...
let within (t:t) elt : (int * int) list =
...
let within_pt (t:t) (elt:int) = 
  let cmp_for_monster elt (i1,i2) = 
    if (elt >= i1) && (elt <= i2) then true else false in
  let rec aux acc t = 
    match t with
      | Leaf -> acc
      | Node (mid,monster,left,right) ->
          match Pervasives.compare elt mid with
            | -1 -> 
                aux (List.append 
                  (IntMonster.within cmp_for_monster monster elt) acc) left
            |  0 -> 
                 List.append (IntMonster.within cmp_for_monster monster elt) acc
            |  1 -> 
                 aux (List.append 
                   (IntMonster.within cmp_for_monster monster elt) acc) right
            |  _ -> failwith "Error in function within_pt."
  in
  aux [] t
